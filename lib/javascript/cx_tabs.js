...
function selectTab(tabName, updateLegend = false, formId = 0)
{
    if(document.getElementById(tabName).style.display != "block")
    {
        ...
        if (updateLegend) {
            ...
            if (formId > 0 ) {
                legend = document.querySelectorAll('#form-' + formId + ' legend');
            }
        ...
        for (i=0;i<arrTags.length;i++)
        {
            ...
            {
                console.log(arrTags[i].id);
                console.log(arrTags[i].id.split('-'));
                console.log(formId);
                if (formId == arrTags[i].id.split('-')[1]) {
