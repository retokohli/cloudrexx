...
function checkaddContact() {
    ...
    if($J("#contact_email").exists() && checkEmail($J("#contact_email").val()) == false) {
        errChk = 1;
        $J("#contact_email").css("border", "1px solid red");
    }
    if (contact_id == 0) {
        if($J("#contact_password").exists() && $J.trim($J("#contact_password").val()) == "") {
            errChk = 1;
...
function checkUserNameAvailablity(customer_id, email) {
    $J.ajax({
       ...
       success    : function(json) {
           ...
           var html = '';
           if (json['error']) {
                html = "<span class='error'>"+ json['error'] +"</span>";
           }
           if (json['success']) {
