    ...
    $J('#frmImport2').bind('submit', function(e) {
        ...
        xhr = $J.ajax({
                ...
                beforeSend: function() {
                    ...
                    refreshIntervalId = self.setInterval(function(){getUploadProcess();}, 5000);
                },
...
function getUploadProcess() {
    ...
    if (fileUrl != '') {
        $J.ajax({
            dataType: "json",
            url     : "index.php?cmd=crm&act=interface&tpl=getprogress&file="+fileUrl,
            success : function (data) { 
                        var percent = parseInt(((data.skippedRows + data.importedRows) / totalRows) * 100);
