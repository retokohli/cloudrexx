...
$(function() {
    ...
    $("input.date").datepicker({ dateFormat: 'dd.mm.yy',showOn: "button",
			buttonImage: "template/ascms/images/calender.png",buttonImageOnly: true },$.datepicker.regional['']);
    var config = {
        toolbar:
                [
                    ['Format'],
    ...
    $('#cm_ckeditor').ckeditor(config);

    if ($("#site")) {
        $("#site").tabs().css('display', 'block');
    }
    if ($('#showHideInfo')) {
        $('#showHideInfo').toggle(function() {
            var addInfo = $('#additionalInfo');
            addInfo.css({'height':'auto'});
            $(this).css({'backgroundPosition':'50% 3px'});
            var height = addInfo.height();
            addInfo.css('height','38px');
            addInfo.stop().animate({'height':height}, 500);
        }, function() {
            var addInfo = $('#additionalInfo');
            $(this).css({'backgroundPosition':'50% -29px'});
            addInfo.stop().animate({'height':'38px'}, 500);
        });
    }
    ...
    $("#site-tree")
                ...
                "types" : {
                    ...
                    "types" : {
                        ...
                        "default" : {
                            ...
                            "icon" : {
                                "image" : "template/ascms/images/file.png"
                            }
                        },
                        // The `folder` type
                        "folder" : {
                            ...
                            "icon" : {
                                "image" : "template/ascms/images/folder.png"
                            }
                        },
                        // The `drive` nodes
                        "drive" : {
                            ...
                            "valid_children" : [ "default", "folder" ],
                            "icon" : {
                                "image" : "template/ascms/images/root.png"
                            },
		...
		$('#site-tree a').each(function(index, leaf) {
			$(leaf).click(function() {
				// this.id == page_id
				//CKEDITOR.instances.cm_ckeditor.setData(this.id);
				$.ajax({url : '?cmd=jsondata&id='+this.id, complete : function(response) { CKEDITOR.instances.cm_ckeditor.setData(response.responseText) }});
			});
		});
	    });
    ...
    $(".chzn-select").chosen().change(function(){ 
        var str = "";
         $("select.chzn-select option:selected").each(function () {
            str += $(this).attr('value');
         });
        $("#site-tree").jstree("set_lang", str);
        $("input.date").datepicker($.datepicker.regional[str]);
    });
});
