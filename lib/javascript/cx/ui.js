    ...
    function() {
        ...
        var UI = function() {
            ...
            var intro = {
                 ...
                 */
                show: function(group) {
                    updates = cx.variables.get("steps", "View/intro");
                    if (!updates) {
                        return;
                    }
                    if (group == undefined) {
                        cx.jQuery.each(updates, function(index, el) {
                            cx.ui.intro.show(index);
                        });
                        return;
                    }
                    if (!updates[group]) {
                    ...
                    updates = updates[group];

                    // get shown steps from cookie
                    ...
                    var updateLevel = getCookie("view-intro-" + group);
                    if (!updateLevel) {
                        updateLevel = 0;
                    }
                    // refresh cookie
                    setCookie("view-intro-" + group, updateLevel, 30);

                    // buildup array and if time is greater than any of the array: skip
                    var steps = [];
                    var newUpdateLevel = updateLevel;
                    cx.jQuery.each(updates, function(index, el) {
                        if (updateLevel & (1<<index)) {
                            return true; // continue
                        }
                        if (!cx.jQuery(el.element).length) {
                            return true; // continue
                        }
                        newUpdateLevel = newUpdateLevel | (1<<index);
                        steps.push(el);
                    });
                    if (!steps.length) {
                        return;
                    ...
                    const intro = introJs();
                    intro.setOptions({
                        nextLabel: cx.variables.get("TXT_CORE_INTRO_NEXT", "Core/lang"),
                        prevLabel: cx.variables.get("TXT_CORE_INTRO_BACK", "Core/lang"),
                        skipLabel: cx.variables.get("TXT_CORE_INTRO_STOP", "Core/lang"),
                        doneLabel: cx.variables.get("TXT_CORE_INTRO_STOP", "Core/lang"),
                        showStepNumbers: false,
                        tooltipPosition: "auto",
                        steps: steps
                    });
                    introJs.fn.oncomplete(function() {
                        setCookie("view-intro-" + group, newUpdateLevel, "30");
                    });
                    intro.start();
                }
            }
