...
var ContrexxJs = function(theOptions) {
     ...
     * @param boolean internal (DO NOT USE) undefined by default, used to determine user call
     ...
     */
    var callThreadSafeFunc = function(callback, scope, sleep, internal) {
        if (sleep == undefined) {
            sleep = 500;
        }
        var funcId = scope;
        if (funcId == undefined) {
            funcId = callback.toString();
        }
        if (!internal) {
        ...
        if (threadSafeCallBlocks[funcId]) {
            setTimeout(function() {
                callThreadSafeFunc(callback, funcId, sleep, true);
            }, sleep);
        } else {
