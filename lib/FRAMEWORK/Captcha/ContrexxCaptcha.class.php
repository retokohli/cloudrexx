...
class ContrexxCaptcha implements CaptchaInterface {
    ...
    public function getCode($tabIndex = null)
    {
        ...
        $code = '<span class="captcha">';
        $code .= '<label for="coreCaptchaCode" class="coreCaptchaLabel">'.$_CORELANG['TXT_CORE_CAPTCHA_ENTER_THE_LETTERS_BELOW'].'</label>';
        $code .= '<input type="text" name="coreCaptchaCode" class="coreCaptchaCode" value="" maxlength="5" '.$tabIndexAttr.' autocomplete="off" />';
        $code .= '<img src="'.$this->getUrl().'" alt="'.$alt.'" class="coreCaptchaImage" />';
        $code .= '</span>';
