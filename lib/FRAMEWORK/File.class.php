    ...
    static function init()
    {
        ...
        if (self::$connection) {
            ...
            if (@ftp_login(
                  self::$connection,
                  $_FTPCONFIG['username'],
                  $_FTPCONFIG['password'])) return true;
...
//DBG::log("File::make_folder_ftp($folder_path): Failed to create folder ".self::$ftpPath."/$folder_path<br />");
    ...
    function delete_folder($folder_path, $force=false)
    {
        ...
        while ($file !== false) {
            ...
            if (is_file(ASCMS_DOCUMENT_ROOT.'/'.$file_path)) {
                ...
                if (!self::delete_folder($file_path)) return false;
            }
            $file = @readdir($resource);
        }
        closedir($resource);
    ...
    {
        self::path_relative_to_root($file_path);
//DBG::log("File::delete_file($file_path): File: ".ASCMS_DOCUMENT_ROOT.'/'.$file_path);
        if (!self::exists($file_path)) {
//DBG::log("File::delete_file($file_path): File does not exist: ".ASCMS_DOCUMENT_ROOT.'/'.$file_path);
            return true;
        }
        ...
        clearstatcache();
        if (self::exists($file_path)) {
//DBG::log("File::delete_file($file_path): Failed to delete, trying FTP: ".ASCMS_DOCUMENT_ROOT.'/'.$file_path);
