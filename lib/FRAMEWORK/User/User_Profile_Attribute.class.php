...
class User_Profile_Attribute
{
	...
	var $arrAttributeTree;
	var $arrAttributeRelations;
	var $arrCoreAttributeIds;
	var $arrCustomAttributes;
	var $arrMandatoryAttributes = array();

	var $arrCoreAttributes = array(
		'picture' => array(
			'type'				=> 'image',
			'multiline'			=> false,
			'mandatory'			=> false,
	...
	function loadCoreAttributesCustomizing()
	{
		...
		$objAttribute = $objDatabase->Execute('SELECT `id`, `mandatory`, `sort_type`, `order_id`, `access_special`, `access_id` FROM `'.DBPREFIX.'access_user_core_attribute`');
		if ($objAttribute) {
