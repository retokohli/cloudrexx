...
//    }
    ...
    function addUser()
    {
        ...
        if (   strlen($_POST['password']) >= 6
            ...
            if (userManagement::addUserByParam(
                $_POST['username'],
                (isset($_POST['password']  ) ? $_POST['password']   : ''),
                $_POST['email'],
                (isset($_POST['is_admin']  ) ? $_POST['is_admin']   : ''),
                true,
                (isset($_POST['dest']      ) ? $_POST['dest']       : ''),
                0, // Validity period defaults to unlimited
                (isset($_POST['adminlang'] ) ? $_POST['adminlang']  : ''),
                ...
                (isset($_POST['webpage']   ) ? $_POST['webpage']    : ''),
                (isset($_POST['profession']) ? $_POST['profession'] : ''),
                (isset($_POST['interests'] ) ? $_POST['interests']  : '')
            ) > 0) {
     ...
     * @global    object     $objDatabase   Database object
     * @param     string     $username      The user name
     * @param     string     $password      The password
     * @param     string     $email         The e-mail address
     * @param     boolean    $isAdmin       The user is an administrator iff true
     * @param     boolean    $isActive      The user is active iff true
     * @param     array      $arrGroup      The user group IDs the user belongs to
     * @param     integer    $langId        The users' language ID
     ...
     * @param     string     $street        The optional street name and number
     * @param     string     $zip           The optional zip number
     * @param     string     $residence     The optional city of residence
     * @param     string     $phone         The optional phone number
     * @param     string     $mobile        The optional cell phone number
     * @param     string     $webpage       The optional web address URI
     * @param     string     $profession    The optional profession
     * @param     string     $interests     The optional interests
     * @return    integer                   The ID of the user created
     *                                      on success, 0 (zero) otherwise
     * @author  Reto Kohli <reto.kohli@comvation.com>
     */
    //static
    function addUserByParam(
        $username, $password, $email,
        $isAdmin=false, $isActive=true, $arrGroup=array(),
        $validity,
        $langId=0,
        $firstname='', $lastname='', $company='',
        $street='', $zip='', $residence='',
        $phone='', $mobile='',
        $webpage='', $profession='', $interests='')
    {
        ...
        if (   strlen($password) >= 6
            ...
            // Add standard public group if there is none present
            $groups = (count($arrGroup) > 0 ? implode(',', $arrGroup) : '2');
            $isAdmin = ($isAdmin ? 1 : 0);
            $isActive = ($isActive ? 1 : 0);
            $query = "
                INSERT INTO ".DBPREFIX."access_users
                   SET username='".addslashes(strip_tags($username))."',
                       password='$password',
                       ...
                       company='".addslashes(strip_tags($company))."',
                       street='".addslashes(strip_tags($street))."',
                       zip='".addslashes(strip_tags($zip))."',
                       residence='".addslashes(strip_tags($residence))."',
                       phone='".addslashes(strip_tags($phone))."',
                       mobile='".addslashes(strip_tags($mobile))."',
                       webpage='".addslashes(strip_tags($webpage))."',
                       profession='".addslashes(strip_tags($profession))."',
                       interests='".addslashes(strip_tags($interests))."',
                       regdate=CURDATE(),
                       validity=$validity
            ";
...
echo("Query error, code dfgskljrthlw:<br />$query<br />");
        }
        return 0;
    }
    ...
    function showNewUser()
    {
        ...
        $objTemplate->setVariable(array(
            ...
            'USERS_VALIDITY_MENU_OPTIONS' =>
                self::getValidityMenuOptions($objResult->fields['validity']),
        ));
...
*/
    ...
    {
        global $objDatabase, $_CORELANG, $objTemplate, $objInit;
            ...
            foreach ($arrModules as $id => $module) {
                if (in_array(DBPREFIX."module_".$module['name']."_access", $arrTables)) {
                    ...
                    if ($objResult !== false && $objResult->RecordCount()>0) {
                        while (!$objResult->EOF) {
                            $arrModuleFunctions[$id][$objResult->fields['access_id']] = $objResult->fields['description'];
                            $objResult->MoveNext();
                        }
        ...
        if (isset($_GET['groupId']) AND intval($_GET['groupId'])!=0) {
            ...
            if ($objResult !== false) {
                while (!$objResult->EOF) {
                    $userList[$objResult->fields['id']] = $objResult->fields['username'];
...
//                'TR_STATUS'                => $visible_tr,
                ...
                'USERS_USERNAME'        => $objResult->fields['username'],
                'USERS_PASSWORD'        => '',
                'USERS_EMAIL'            => $objResult->fields['email'],
                'USERS_FIRSTNAME'        => $objResult->fields['firstname'],
                'USERS_LASTNAME'        => $objResult->fields['lastname'],
                'USERS_LASTNAME'        => $objResult->fields['lastname'],
                'USERS_RESIDENCE'        => $objResult->fields['residence'],
                'USERS_PROFESSION'        => $objResult->fields['profession'],
                'USERS_INTERESTS'        => $objResult->fields['interests'],
                'USERS_WEBPAGE'            => $objResult->fields['webpage'],
                'USERS_COMPANY'            => $objResult->fields['company'],
                'USERS_VALIDITY_MENU_OPTIONS' =>
                    self::getValidityMenuOptions($objResult->fields['validity']),
            ));
// TODO: unused
//            $status = $objResult->fields['status'];
    ...
    function userOverview()
    {
        ...
        if (isset($_REQUEST['useract']) && $_REQUEST['useract'] == "inactive") {
            $status = 0;
        ...
        $arrGroups = $this->_getGroups();
        $_SESSION['usersearch']['groupfilter'] =
            (isset($_SESSION['usersearch']['groupfilter'])
        ...
        $_SESSION['usersearch']['criteria'] =
            (isset($_SESSION['usersearch']['criteria'])
                ? contrexx_addslashes(trim($_REQUEST['user_search_criteria']))
                : ''
            );
        $_SESSION['usersearch']['searchterm'] =
            (isset($_SESSION['usersearch']['searchterm'])
        ...
        switch ($_REQUEST['by']) {
            ...
            break;
            case 'validity':
                $sort = " ORDER BY validity ".$_SESSION['usersearch']['sort'][$_REQUEST['by']];
        }
        ...
        $objTemplate->setVariable(array(
            ...
            'USER_SEARCH_SELECTED_'.strtoupper($searchCriteria) => 'selected="selected"',
        ...
        /** start paging **/
        $query = "
            SELECT u.id, u.is_admin, u.username, u.email, u.firstname,
                   u.lastname, u.groups, l.name AS language, validity, regdate
        ...
        if ($count>intval($_CONFIG['corePagingLimit'])) {
            $queryString =
                '&amp;user_search_criteria='.$searchCriteria.
        ...
        if ($objResult) {
            while (!$objResult->EOF) {
                ...
                $validity = $objResult->fields['validity'];
                $endDate =
                    ($validity > 0 ? date('d.m.Y', ($orderUnixTimeStamp+($validity * 24 * 60 * 60))) : '');

                $objTemplate->setVariable(array(
                    'USERS_ROWCLASS'     => (++$i % 2 ? 'row2' : 'row1'),
                    'USERS_EDIT_ID'      => $id,
                    'USERS_ID'           => $id,
                    'USERS_USERNAME'     => $objResult->fields['username'],
                    'USERS_EMAIL'        => $objResult->fields['email'],
                    'USERS_FIRSTNAME'    => empty($objResult->fields['firstname']) ? "&nbsp;" : $objResult->fields['firstname'],
                    'USERS_LASTNAME'     => empty($objResult->fields['lastname']) ? "&nbsp;" : $objResult->fields['lastname'],
                    'USERS_JOIN_DATE'    => date(ASCMS_DATE_SHORT_FORMAT, strtotime($objResult->fields['regdate'])),
                    'USERS_LANGUAGE'     => $objResult->fields['language'],
                    'USERS_ADMIN_STATUS' => $is_admin,
                    'USERS_VALIDITY' =>
                        userManagement::getValidityString($validity).
                        ($endDate ? " ($endDate)" : ''),
                ));
                if ($status == 1) {
    ...
    /**
     * Returns HTML code for a dropdown menu containing all frontend
     * user group names with their IDs as values.
     * @static
     * @param   integer   $selected   The optional preselected user group ID
     * @return  string                The dropdown menu HTML code on success,
     *                                the empty string ('') otherwise
     * @author  Reto Kohli <reto.kohli@comvation.com>
     */
    //static
    function getFrontendGroupMenu($selected=0)
    {
        global $objDatabase, $_CORELANG;
        ...
        $strMenu =
            ...
            '<option value="0"'.'>'.
            $_CORELANG['TXT_USERGROUP_DONTCARE'].
            '</option>';
        while (!$objResult->EOF) {
    ...
    /**
     * Returns an array of all active frontend groups
     ...
     *  )
     * @static
     * @return  array                 The array of active frontend groups
     *                                on success, false otherwise.
     * @author  Reto Kohli <reto.kohli@comvation.com>
     */
    //static
    function getActiveFrontendGroupArray()
    {
        global $objDatabase;
        ...
        $arrActiveFrontendGroups = array();
        while (!$objResult->EOF) {
            $id = $objResult->fields['group_id'];
            $name = $objResult->fields['group_name'];
            $arrActiveFrontendGroups[$id] = $name;
            $objResult->MoveNext();
        }
        return $arrActiveFrontendGroups;
    }


    /**
     * Returns an array with some essential user account information.
     *
     * Used by the Shop in order to let the customer check the validity
     * of her account.
     * Note that this is a noser.com special, where both customer and
     * user accounts have the same login names and password.
     * @static
     * @param   string      $username       The user name
     * @return  mixed                       The user info array on success,
     *                                      false otherwise
     * @author  Reto Kohli <reto.kohli@comvation.com>
     */
    function getUserInfoById($id)
    {
        ...
        $query = "
            SELECT id, username, password,
                   firstname, lastname, email, regdate, validity
              FROM ".DBPREFIX."access_users
             WHERE id=$id
        ";
        $objResult = $objDatabase->Execute($query);
    ...
    function getUserInfo()
    {
        ...
        return self::getUserInfoById($objAuth->getUserId());
    }


    /**
     * Updates the user password for the given user ID.
     *
     * Note: This method will return false if either the ID or
     * password are empty, if it fails to update the record, or if
     * the number of rows affected by the update is zero.
     * The latter is the case with most implementations.
     * @static
     * @global  mixed     $objDatabase    Database object
     * @global  array     $_CORELANG      Language array
     * @return  boolean                   True on success, false otherwise
     * @author  Reto Kohli <reto.kohli@comvation.com>
     */
    //static
    function changePasswordById($id, $password)
    {
        global $objDatabase, $_CORELANG;
    ...
    /**
     * Delete the user account whose user name matches the given order ID.
     *
     * Order IDs have the form "A-08-8nnnn", and user names are formed by
     * adding the Product ID and an instance number, like ""A-08-8nnnn-x-y".
     * Thus, any accounts matching the order ID plus a trailing dash are
     * deleted.
     * @param   string    $orderId    The Noser order ID,user name
     * @return  boolean               True on success, false otherwise
     * @global  mixed     $objDatabase    Database object
     */
    function deleteUserByOrderId($orderId)
    {
        global $objDatabase;

        $query = "
            DELETE FROM ".DBPREFIX."access_users
             WHERE username LIKE '".addslashes($orderId)."-%'
        ";
        $objResult = $objDatabase->Execute($query);
        if (!$objResult) {
            return false;
    ...
    /**
     * Returns the HTML dropdown menu options for choosing the validity period.
     * @global  mixed     $objDatabase        Database object
     * @global  array     $_CORELANG          Language array
     * @param   integer   $selectedValidity   The optional preselected validity
     */
    function getValidityMenuOptions($selectedValidity=0)
    {
        global $objDatabase;

        $query = "
            SELECT validity
              FROM ".DBPREFIX."user_validity
          ORDER BY validity ASC
        ";
        $objResult = $objDatabase->Execute($query);
        if (!$objResult) {
            userManagement::fixValidityTable();
            return '';
        }
        ...
        while (!$objResult->EOF) {
            $validity = $objResult->fields['validity'];
            $strValidity = userManagement::getValidityString($validity);
            $strOptions .=
...
//echo(htmlentities($strOptions)."<br />");
        return $strOptions;
    }


    function fixValidityTable()
    {
        global $objDatabase;

        $query = "
            CREATE TABLE ".DBPREFIX."user_validity (
        ...
        $query = "
            INSERT INTO ".DBPREFIX."user_validity
              (validity)
            VALUES
              (0), (1), (15), (31), (62), (92), (123), (184), (366), (731)
        ";
        $objDatabase->Execute($query);
    }
}
