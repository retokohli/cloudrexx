 ...
 * @todo        Edit PHP DocBlocks!
 */
class YamlSetting extends Engine{
    /**
     * Path to the configuration file used as storage location
     * @var string
     */
    static $filename = null;
    static $modelSettingRepo = null;
    ...
    static function init($section, $group=null, $configRepository = null) {
        ...
        if (!isset(self::$modelSettingRepo)) {
            self::$modelSettingRepo = new \Cx\Core\Setting\Model\Repository\ModelSettingRepository();
        }
        self::$modelSettingRepo->initialize(self::$filename);
        self::$arrSettings = self::load();
    }

    static function load() {
        if (!empty(self::$modelSettingRepo)) {
            $modelSettings = self::$modelSettingRepo->findAll();
            if (isset($modelSettings)) {
                foreach ($modelSettings As $modelSetting) {
                    $modelSettingArray[$modelSetting->getName()] = array(
                        'name'    => $modelSetting->getName(),
                        'section' => $modelSetting->getSection(),
                        'group'   => $modelSetting->getGroup(),
                        'value'   => $modelSetting->getValue(),
                        'type'    => $modelSetting->getType(),
                        'values'  => $modelSetting->getValues(),
                        'ord'     => $modelSetting->getOrd()
                    );
                }
                return $modelSettingArray;
            }
        }
        return false;
    }
    ...
    { 
        ...
        $success = true;
        try {
            foreach (self::$arrSettings As $modelSettingName => $modelSettingValue) {
                $objModelSetting = self::$modelSettingRepo->findOneBy(array('name' => $modelSettingName, 'section' => self::section));
                $objModelSetting->setValue($modelSettingValue['value']);
            
                \Env::get('em')->persist($objModelSetting);
            }
            \Env::get('em')->flush();
        } catch (\Cx\Core\Setting\Model\Entity\ModelSettingException $e) {
    ...
    static function update($name)
    {
        ...
        if (!empty(self::$arrSettings)) {
            try {
                $objModelSetting = self::$modelSettingRepo->findOneBy(array('name' => $name, 'section' => self::$section));
                $objModelSetting->setValue(self::$arrSettings[$name]['value']);
    ...
    static function add( $name, $value, $ord=false, $type='text', $values='', $group=null)
    {
        ...
        try {
            $objModelSetting = new \Cx\Core\Setting\Model\Entity\ModelSetting($name);
            $objModelSetting->setSection(self::$section);
            $objModelSetting->setGroup($group);
            $objModelSetting->setValue($value);
            $objModelSetting->setValue($value);
            $objModelSetting->setType($type);
            $objModelSetting->setValues($values);
            $objModelSetting->setOrd($ord);
    ...
    { 
        ...
        try {
            if (!empty($name) && !empty($group)) {
                $modelSettings = self::$modelSettingRepo->findBy(array('name' => $name, 'group' => $group, 'section' => self::$section));
            } else if (!empty ($name)) {
                $modelSettings = self::$modelSettingRepo->findBy(array('name' => $name, 'section' => self::$section));
            } else if (!empty ($group)) {
                $modelSettings = self::$modelSettingRepo->findBy(array('group' => $group, 'section' => self::$section));
            }
        
            foreach ($modelSettings As $modelSetting) {
                \Env::get('em')->remove($modelSetting);
            }
            \Env::get('em')->flush();
        } catch (\Cx\Core\Setting\Model\Entity\ModelSettingException $e) {
            \DBG::msg($e->getMessage());
    ...
    static function deleteModule()
    {
        ...
        try {
            $modelSettings = self::$modelSettingRepo->findBy(array('section' => self::$section));
            foreach ($modelSettings As $modelSetting) {
                \Env::get('em')->remove($modelSetting);
            }
            \Env::get('em')->flush();
