...
class InitCMS
{
    private $currentThemesId;
    public $is_home;
    private $arrLang = array();
    private $arrLangNames = array();
    private $templates = array();
    private $arrModulePath = array();
     ...
     * Constructor
     *
     * Initializes the language arrays and constants (DEFAULT_LANG_ID,
     * FRONTEND_LANG_ID, LANG_ID, and BACKEND_LANG_ID (the latter is
     * defined in backend mode only).
     * Initializes the module path array
     */
        ...
        if ($objResult) {
            while (!$objResult->EOF) {
                ...
                if ($objResult->fields['is_default'] == 'true') {
                    define('DEFAULT_LANG_ID', $objResult->fields['id']);
                }
                $objResult->MoveNext();
            }
        }
        if ($mode == 'backend') {
            define('BACKEND_LANG_ID', $this->_initBackendLanguage());
            define('LANG_ID', BACKEND_LANG_ID);
            define('FRONTEND_LANG_ID', $this->getUserFrontendLangId());
        } else {
            define('FRONTEND_LANG_ID', $this->_initFrontendLanguage());
     ...
     *
     * @return  integer   The backend language ID
     */
    ...
    function _initFrontendLanguage()
    {
        ...
        if ($this->arrLang[$frontendLangId]['frontend'] != 1) {
            $frontendLangId = DEFAULT_LANG_ID;
        }
        if ($setCookie) {
    ...
    function getUserFrontendLangId()
    {
        ...
        $_SESSION['userFrontendLangId'] = $frontendLangId;
        return $frontendLangId;
    }
    ...
    {
        return $this->arrLang[FRONTEND_LANG_ID]['lang'];
    }
    ...
    {
        $charset = $this->arrLang[FRONTEND_LANG_ID]['charset'];
    ...
    function getTemplates()
    {
        ...
        $this->templates['subnavbar2'] =
            (file_exists(ASCMS_THEMES_PATH.'/'.$themesPath.'/subnavbar2.html')
                ? file_get_contents(ASCMS_THEMES_PATH.'/'.$themesPath.'/subnavbar2.html')
                : ''
            );
        $this->templates['subnavbar3'] =
            (file_exists(ASCMS_THEMES_PATH.'/'.$themesPath.'/subnavbar3.html')
                ? file_get_contents(ASCMS_THEMES_PATH.'/'.$themesPath.'/subnavbar3.html')
                : ''
            );
        ...
        $this->templates['buildin_style'] =
            (file_exists(ASCMS_THEMES_PATH.'/'.$themesPath.'/buildin_style.css')
                ? file_get_contents(ASCMS_THEMES_PATH.'/'.$themesPath.'/buildin_style.css')
                : ''
            );
    ...
    function loadLanguageData($module='')
    {
        ...
        if (!array_key_exists($module, $objInit->arrModulePath) && $module != 'media') {
            ...
            $path = $objInit->arrModulePath[$module].$objInit->arrLang[$langId]['lang'].'/'.$objInit->mode.'.php';
               if (!file_exists($path)) {
                ...
                //$objInit->mode == 'backend' ? $objInit->getBackendDefaultLangId() : $objInit->getFrontendDefaultLangId();
                $path = $objInit->arrModulePath[$module].$objInit->arrLang[$langId]['lang'].'/'.$objInit->mode.'.php';
                if (!file_exists($path)) {
                    $path = '';
                }
        ...
        // load variables
        if (empty($module)) return $_CORELANG;
        if (!empty($path)) {
            //require_once($path);
            require($path);
    ...
    /**
     * Returns the HTML code for the frontend language selection menu
     *
     * Backend use only.  May be shown on any page that needs it.
     * @return  string                  The HTML dropdown menu code
     * @todo    This should be adapted to ignore unwanted query
     *          parameters and not add them to the form action attribute
     */
    function getUserFrontendLangMenu()
    {
        global $_CORELANG;

        // Do not show the menu if there's less than two active languages
        // TODO:  This is not quite correct, as it does not consider the
        // frontend and backend flags.  Ignored for now.
        //if (count($this->arrLang) < 2) return '';
        $arrVars = array();
        if (isset($_SERVER['QUERY_STRING'])) {
            parse_str($_SERVER['QUERY_STRING'], $arrVars);
        }
// Old version:  cmd parameter only
//        $query = (isset($arrVars['cmd']) ? '?cmd='.$arrVars['cmd'] : '');
// New version:  Consider all parameters from the query string
        $query = '';
        foreach ($arrVars as $name => $value) {
            $query .=
                ($query ? '&amp;' : '?').$name.'='.$arrVars[$name];
        }
        $strMenu =
            "\n".'<form action="index.php'.$query.
            '" method="post" name="userFrontendLangIdForm">'."\n".
            $_CORELANG['TXT_CORE_LANGUAGE_FRONTEND_COLON'].
            '&nbsp;<select name="userFrontendLangId" size="1" '.
            'onchange="document.forms[\'userFrontendLangIdForm\'].submit()">'."\n";
        foreach($this->arrLang as $id => $value) {
            if ($this->arrLang[$id]['frontend']) {
                $strMenu .=
                    '<option value="'.$id.'"'.
                    ($id == FRONTEND_LANG_ID ? ' selected="selected"' : '').
                    '>'.htmlentities($value['name'], ENT_QUOTES, CONTREXX_CHARSET).
                    "</option>\n";
            }
        }
        $strMenu .= "</select>\n</form>\n";
