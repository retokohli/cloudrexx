...
/**
 * @internal  Fixes the code analyzer warning
 */
global $objInit;
if (isset($objInit) && $objInit->mode == 'backend') {
    /**
     * @ignore
     */
...
class settingsManager
{
    ...
    function getPage()
    {
        ...
        $objTemplate->setVariable('CONTENT_NAVIGATION',
            '<a href="?cmd=settings">'.$_CORELANG['TXT_SETTINGS_MENU_SYSTEM'].'</a>'.
    ...
    function showSettings()
    {
        ...
        $objResult = $objDatabase->Execute('
            SELECT setid, setname, setvalue, setmodule
        ...
        $objTemplate->setVariable(array(
            ...
            'TXT_CORE_USE_VIRTUAL_LANGUAGE_PATH_HELP' =>
                sprintf(
                    $_CORELANG['TXT_CORE_USE_VIRTUAL_LANGUAGE_PATH_HELP'],
                    htmlentities(FWLanguage::getLanguageParameter($_FRONTEND_LANGID, 'name'),
                        ENT_QUOTES, CONTREXX_CHARSET),
                    ASCMS_PROTOCOL.'://'.$_CONFIG['domainUrl'].ASCMS_PATH_OFFSET.'/'.
                    FWLanguage::getLanguageParameter(FRONTEND_LANG_ID, 'lang').'/').
                    (   $arrSettings['useVirtualLanguagePath'] == 'on'
                     || $this->checkForVirtualLanguagePathSupport()
                        ? ''
                        : '<br /><strong>'.$_CORELANG['TXT_CORE_APACHE_MOD_REWRITE_REQUIRED'].'</strong>'),
     ...
     *
     * Checks if the system is running on an Apache webserver
     * and if the mod_rewrite module is loaded.
     * @return boolean
     */
    private function checkForVirtualLanguagePathSupport()
    {
        ...
        if ($objFWHtAccess->loadHtAccessFile('/.htaccess', false) === true && $objFWHtAccess->isRewriteEngineInUse())
            return true;
        if ($objFWHtAccess->checkForApacheServer() && $objFWHtAccess->checkForModRewriteModul())
            return true;
        return false;
    }
    ...
    public function setVirtualLanguagePath($use)
    {
        ...
        foreach ($arrLanguages as $arrLanguage) {
            if (!$arrLanguage['frontend']) continue;
    ...
    function writeSettingsFile()
    {
        ...
        $handleFile = false;
        if ($this->isWritable()) $handleFile = fopen($this->strSettingsFile,'w+');
        if (!$handleFile) {
            $this->strOkMessage = '';
        ...
        // Get values
        $objResult = $objDatabase->Execute('
            SELECT setname, setmodule, setvalue
              FROM '.DBPREFIX.'settings
             ORDER BY setmodule ASC, setname ASC');
        $intMaxLen = 0;
        if ($objResult->RecordCount() > 0) {
            while (!$objResult->EOF) {
    ...
    function smtp()
    {
        if (empty($_REQUEST['tpl'])) $_REQUEST['tpl'] = '';
        switch ($_REQUEST['tpl']) {
            case 'modify':
                $this->_smtpModify();
                break;
    ...
    function _smtpModify()
    {
        ...
        if (isset($_POST['settings_smtp_save'])) {
            $arrSmtp = array(
                'name'     => (empty($_POST['settings_smtp_account'])
                    ? '' : contrexx_stripslashes(trim($_POST['settings_smtp_account']))),
                'hostname' => (empty($_POST['settings_smtp_hostname'])
                    ? '' : contrexx_stripslashes(trim($_POST['settings_smtp_hostname']))),
                'port'     => (empty($_POST['settings_smtp_port'])
                    ? 25 : intval($_POST['settings_smtp_port'])),
                'username' => (empty($_POST['settings_smtp_username'])
                    ? '' : contrexx_stripslashes(trim($_POST['settings_smtp_username']))),
                'password' => (empty($_POST['settings_smtp_password'])
                    ? '' : contrexx_stripslashes($_POST['settings_smtp_password'])),
            );
            if (!$arrSmtp['port']) $arrSmtp['port'] = 25;
        ...
        $objTemplate->setVariable(array(
            'SETTINGS_SMTP_TITLE'    => ($id
                ? $_CORELANG['TXT_SETTINGS_MODIFY_SMTP_ACCOUNT']
                : $_CORELANG['TXT_SETTINGS_ADD_NEW_SMTP_ACCOUNT']),
            'SETTINGS_SMTP_ID'       => $id,
            'SETTINGS_SMTP_ACCOUNT'  => htmlentities($arrSmtp['name'], ENT_QUOTES, CONTREXX_CHARSET),
    ...
    function useOwnCSS()
    {
        ...
        if ($objResult) $useCSS = $objResult->fields['setvalue'];

        //get default theme
        if ($useCSS == "on") {
            $query = "
                SELECT themesid, foldername
                  FROM ".DBPREFIX."languages
                  LEFT JOIN ".DBPREFIX."skins
                       ON ".DBPREFIX."skins.id=".DBPREFIX."languages.themesid
                 WHERE ".DBPREFIX."languages.is_default = 'true';";
            $objResult = $objDatabase->Execute($query);
