...
class Cx {
     ...
     * Initializes the CMS
     */
    public function __construct($frontend = true) {
        ...
        $this->init($frontend);         // ClassLoader, API, DB (incl. Doctrine)
        $this->postInit($frontend);     // Components
        ...
        // @todo move to somewhere else
            ...
            */
            $objTemplate = new \Cx\Core\Html\Sigma($frontend ? ASCMS_THEMES_PATH : ASCMS_ADMIN_TEMPLATE_PATH);
        ...
        $this->loadContent($frontend);       // Init current module
        $this->postContentLoad();   // Call post content load hook scripts
        
        $this->setPostContentLoadPlaceholders($frontend, $objTemplate); // Set Placeholders
        
        $this->finalize($frontend, $objTemplate);          // Set template vars and display content
    }
    ...
    private function postInit($frontend) {
        $this->loadComponents($frontend);
    }
    ...
    private function loadComponents($frontend) {
        global $ch;
        
        $ch = new \Cx\Core\Component\ComponentHandler($frontend);
        $ch->initComponents();
    }
    
    private function init($frontend) {
        ...
        */
        ...
        $objInit = new \InitCMS($frontend ? 'frontend' : 'backend', \Env::em());
...
//            echo $e->getMessage();
    ...
    private function setPostContentLoadPlaceholders($frontend, $objTemplate) {
    ...
    private function finalize($frontend, $objTemplate) {
