...
/**
 * Debug level
 * 1  Show all notices, warnings and errors
...
//-------------------------------------------------------
// Yellowpay:  Restore the originating module (currently only shop or egov),
// by which the payment was initiated.
// Also fix the cmd parameter, make sure it points to "success" for the shop,
// and to "" (the default page) for the egov module.
// Note that cloned modules are not supported yet!
...
$query = "
    SELECT c.content, c.title, c.redirect,
           c.metatitle, c.metadesc, c.metakeys, c.metarobots,
           c.css_name,
           n.catname, n.protected, n.frontend_access_id".
...
if ($objResult === false || $objResult->EOF) {
    ...
    $page_content   =
        ($frontEditing
          ? ($frontEditingContent != ''
              ? $frontEditingContent : $objResult->fields['content']
            )
    ...
    if ($history) {
        ...
        ', 1);
        if ($objPageProtection && $objPageProtection->RecordCount() == 1) {
            $page_protected = 1;
...
//-------------------------------------------------------
  case 'directory':
        ...
        $directory_title = $directory->getPageTitle();
        if (!empty($directory_title)) {
            $page_metatitle = $directory_title;
        }
