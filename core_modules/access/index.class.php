...
class Access extends AccessLib
{
	...
	public function Access($pageContent)
	{
	...
	private function user(&$metaPageTitle, &$pageTitle)
	{
		...
		if ($objUser = $objFWUser->objUser->getUser(!empty($_REQUEST['id']) ? intval($_REQUEST['id']) : 0)) {
	...
	private function settings()
	{
		...
		$this->parseAccountAttributes($objFWUser->objUser, $edit = true);
