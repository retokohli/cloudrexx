...
//        if ($attributeId && ($objAttribute = $objFWUser->objUser->objAttribute->getById($attributeId))) {
    ...
    function _configModifyMails($copy = false)
    {
        ...
        $this->_objTpl->setVariable(array(
            ...
            'ACCESS_MAIL_BODY_HTML'        => $objUserMail->getFormat() != 'text' ? new \Cx\Core\Wysiwyg('access_mail_body_html', $objUserMail->getBodyHtml()) : '<input type="hidden" name="access_mail_body_html" value="'.htmlentities($objUserMail->getBodyHtml(), ENT_QUOTES, CONTREXX_CHARSET).'" />',
