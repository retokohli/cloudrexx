...
class AccessManager extends AccessLib
{
	...
	*/
	...
	function _groupList()
	{
		...
		if ($objFWUser->objGroup->getGroupCount() > $_CONFIG['corePagingLimit']) {
			$this->_objTpl->setVariable('ACCESS_GROUP_PAGING', getPaging($objFWUser->objGroup->getGroupCount($filter), $limitOffset, '&amp;cmd=access&amp;act=group&amp;sort='.$orderDirection.'&amp;by='.$orderBy.'&amp;group_type_filter='.$groupTypeFilter, $_ARRAYLANG['TXT_ACCESS_GROUPS']));
		}
	...
	function _modifyGroup()
	{
		...
		$objFWUser->objGroup->load(isset($_REQUEST['id']) ? intval($_REQUEST['id']) : 0);
		if (isset($_POST['access_save_group']) || isset($_POST['access_change_protection'])) {
			...
			$objFWUser->objGroup->setName(!empty($_POST['access_group_name']) ? trim(contrexx_stripslashes($_POST['access_group_name'])) : '');
			$objFWUser->objGroup->setDescription(!empty($_POST['access_group_description']) ? trim(contrexx_stripslashes($_POST['access_group_description'])) : '');
			$objFWUser->objGroup->setActiveStatus(isset($_POST['access_group_status']) ? (bool)$_POST['access_group_status'] : false);
			$objFWUser->objGroup->setType(!empty($_POST['access_group_type']) ? $_POST['access_group_type'] : '');
			$objFWUser->objGroup->setUsers(isset($_POST['access_group_associated_users']) && is_array($_POST['access_group_associated_users']) ? $_POST['access_group_associated_users'] : array());
			$objFWUser->objGroup->setStaticPermissionIds(isset($_POST['access_area_id']) && is_array($_POST['access_area_id']) ? $_POST['access_area_id'] : array());
			$objFWUser->objGroup->setDynamicPermissionIds(isset($_POST['access_webpage_access_id']) && is_array($_POST['access_webpage_access_id']) ? $_POST['access_webpage_access_id'] : array());

			if (isset($_POST['access_save_group'])) {
				if ($objFWUser->objGroup->store()) {
					$this->arrStatusMsg['ok'][] = $_ARRAYLANG['TXT_ACCESS_GROUP_STORED_SUCCESSFULLY'];
					$this->_groupList();
					return;
				} else {
					$this->arrStatusMsg['error'][] = $objFWUser->objGroup->getErrorMsg();
				}
			} else {
				...
				if (!$this->_changePageProtection($pageId, $protect, $objFWUser->objGroup->getType())) {
			...
			$objFWUser->objGroup->setType(isset($_POST['access_group_type']) ? $_POST['access_group_type'] : '');
		}
		...
		if ($objUser = $objFWUser->objUser->getUsers(null, null, array('username' => 'asc'))) {
			while (!$objUser->EOF) {
		...
		$objResult = $objDatabase->Execute("
			...
			WHERE `is_active` = 1 AND (`scope` = '".$objFWUser->objGroup->getType()."' OR `scope` = 'global')
	...
	private function changeGroupStatus()
	{
		...
		$objFWUser = FWUser::getFWUserObject();
		$objFWUser->objGroup->load($id);
		if ($objFWUser->objGroup->getId()) {
			$objFWUser->objGroup->setActiveStatus(!$objFWUser->objGroup->getActiveStatus());
			if ($objFWUser->objGroup->store()) {
				$this->arrStatusMsg['ok'][] = sprintf($objFWUser->objGroup->getActiveStatus() ? $_ARRAYLANG['TXT_ACCESS_GROUP_ACTIVATED_SUCCESSFULLY'] : $_ARRAYLANG['TXT_ACCESS_GROUP_DEACTIVATED_SUCCESSFULLY'], $objFWUser->objGroup->getName());
			} else {
				$this->arrStatusMsg['error'][] = $objFWUser->objGroup->getErrorMsg();
			}
		} else {
	...
	function _deleteGroup()
	{
		...
		$objFWUser->objGroup->load($id);
		if ($objFWUser->objGroup->getId()) {
			if ($objFWUser->objGroup->delete()) {
				$this->arrStatusMsg['ok'][] = sprintf($_ARRAYLANG['TXT_ACCESS_GROUP_SUCCESSFULLY_DELETED'], $objFWUser->objGroup->getName());
			} else {
				$this->arrStatusMsg['error'][] = $objFWUser->objGroup->getErrorMsg();
			}
		} else {
	...
	private function userList()
	{
		...
		$userCount = $objFWUser->objGroup->getUserCount();
		$userFilter = array();
		if ($groupId) {
		...
		if ($userCount > 0 && ($objUser = $objFWUser->objUser->getUsers($userFilter, $search, $arrOrder, null, $_CONFIG['corePagingLimit'], $limitOffset)) && $userCount = $objUser->getFilteredSearchUserCount()) {
			...
			$this->_objTpl->setVariable('ACCESS_STATUS_MSG', count($search) || $usernameFilter != '' ? $_ARRAYLANG['TXT_ACCESS_NO_USERS_FOUND'] : sprintf($_ARRAYLANG['TXT_ACCESS_NO_USER_IN_GROUP'], '&laquo;'.htmlentities($objFWUser->objGroup->getName(), ENT_QUOTES, CONTREXX_CHARSET).'&raquo;'));
	...
	private function modifyUser()
	{
		...
		if (Permission::hasAllAccess()) {
			$objFWUser->objGroup->loadGroups();
			while (!$objFWUser->objGroup->EOF) {
				$var = in_array($objFWUser->objGroup->getId(), $objUser->getAssociatedGroupIds()) ? 'associatedGroups' : 'notAssociatedGroups';
				$$var .= "<option value=\"".$objFWUser->objGroup->getId()."\">".htmlentities($objFWUser->objGroup->getName(), ENT_QUOTES, CONTREXX_CHARSET)." [".$objFWUser->objGroup->getType()."]</option>\n";
		...
		$this->parseAccountAttributes($objUser, $edit = true);
	...
	private function getMailLanguageMenu($type, $lang, $attrs)
	{
		...
		$objMail = $objDatabase->Execute("SELECT `lang_id` FROM `".DBPREFIX."access_user_mail` WHERE `type` = '".$type."' AND `lang_id` != 0");
		if ($objMail !== false) {
			while (!$objMail->EOF) {
				array_push($arrUsedLangIds, $objMail->fields['lang_id']);
				$objMail->MoveNext();
			}
	...
	private function getMailFormatMenu($selectedFormat, $attrs)
	{
		...
		foreach ($objFWUser->objMail->getFormats() as $format => $formatTxt) {
...
//		$this->attachJavaScriptFunction('SetUrl');
	...
	function _configMails()
	{
		...
		$nr = 1;
		$objFWUser->objMail->loadMails();
		while (!$objFWUser->objMail->EOF) {
			while (!$objFWUser->objMail->languageEOF) {
				$this->_objTpl->setVariable(array(
					'ACCESS_ROW_CLASS_NR'		=> $objFWUser->objMail->getLangId() ? 2 : 1,
					'ACCESS_MAIL_TYPE'			=> $objFWUser->objMail->getType(),
					'ACCESS_MAIL_LANG'			=> $objFWUser->objMail->getLangId(),
					'ACCESS_MAIL_TYPE_TXT'		=> $objFWUser->objMail->getLangId() ? '&rarr; '.$objFWUser->objMail->getTypeDescription() : $objFWUser->objMail->getTypeDescription(),
					'ACCESS_MAIL_TYPE_STYLE'	=> $objFWUser->objMail->getLangId() ? 'text-indent:10px;' : '',
					'ACCESS_MAIL_LANGUAGE'		=> $objFWUser->objMail->getLangId() ? $objLanguage->getLanguageParameter($objFWUser->objMail->getLangId(), 'name') : $_ARRAYLANG['TXT_ACCESS_ALL'],
					'ACCESS_MAIL_SUBJECT'		=> htmlentities($objFWUser->objMail->getSubject(), ENT_QUOTES, CONTREXX_CHARSET)
				));

				if ($objFWUser->objMail->getLangId()) {
				...
				$objFWUser->objMail->nextLanguage();
			}
			$objFWUser->objMail->next();
		}
	...
	function _configModifyMails($copy = false)
	{
		...
		if ($copy) {
			$objFWUser->objMail->load(contrexx_addslashes($_REQUEST['type']));
			$objFWUser->objMail->setLangId(!empty($_REQUEST['access_mail_lang']) ? intval($_REQUEST['access_mail_lang']) : 0);
		} elseif (!$objFWUser->objMail->load(contrexx_addslashes($_REQUEST['type']), !empty($_REQUEST['access_mail_lang']) ? intval($_REQUEST['access_mail_lang']) : null)
		) {
			return $this->_configMails();
		}
		...
		if (isset($_POST['access_save_mail'])|| isset($_GET['access_change_format'])) {
			...
			$objFWUser->objMail->setFormat(!empty($_POST['access_mail_format']) ? $_POST['access_mail_format'] : null);
			$objFWUser->objMail->setSubject(!empty($_POST['access_mail_subject']) ? contrexx_stripslashes($_POST['access_mail_subject']) : '');
			$objFWUser->objMail->setSenderMail(!empty($_POST['access_mail_sender_address']) ? contrexx_stripslashes($_POST['access_mail_sender_address']) : '');
			$objFWUser->objMail->setSenderName(!empty($_POST['access_mail_sender_name']) ? contrexx_stripslashes($_POST['access_mail_sender_name']) : '');
			$objFWUser->objMail->setBodyText(!empty($_POST['access_mail_body_text']) ? contrexx_stripslashes($_POST['access_mail_body_text']) : '');
			$objFWUser->objMail->setBodyHtml(!empty($_POST['access_mail_body_html']) ? contrexx_stripslashes($_POST['access_mail_body_html']) : '');

			if (isset($_POST['access_save_mail'])) {
				if ($objFWUser->objMail->store()) {
					$this->arrStatusMsg['ok'][] = $_ARRAYLANG['TXT_ACCESS_MAIL_STORED_SUCCESSFULLY'];
					return $this->_configMails();
				} else {
					$this->arrStatusMsg['error'] = array_merge($this->arrStatusMsg['error'], $objFWUser->objMail->getErrorMsg());
				}
			}
		}
		...
		if ($copy) {
			if (($language = $this->getMailLanguageMenu($objFWUser->objMail->getType(), $objFWUser->objMail->getLangId(), 'name="access_mail_lang" style="width:400px;"')) === false) {
				$this->_configMails();
				return;
			}
		} elseif (!$objFWUser->objMail->getLangId()) {
			$language = '-';
		} else {
			$language = '<input type="hidden" name="access_mail_lang" value="'.$objFWUser->objMail->getLangId().'" />'.$objLanguage->getLanguageParameter($objFWUser->objMail->getLangId(), 'name');
		}

		$this->_objTpl->setVariable(array(
			'ACCESS_MAIL_ACTION'			=> $copy ? 'copyMail' : 'modifyMail',
			'ACCESS_MAIL_TYPE'				=> $objFWUser->objMail->getType(),
			'ACCESS_MAIL_TYPE_TXT'			=> $objFWUser->objMail->getTypeDescription(),
			'ACCESS_MAIL_LANGUAGE'			=> $language,
			'ACCESS_MAIL_SUBJECT'			=> htmlentities($objFWUser->objMail->getSubject(), ENT_QUOTES, CONTREXX_CHARSET),
			'ACCESS_MAIL_FORMAT'			=> $this->getMailFormatMenu($objFWUser->objMail->getFormat(), 'name="access_mail_format" onchange="document.getElementById(\'access_mail_form\').action=\'index.php?cmd=access&amp;act=config&amp;tpl='.($copy ? 'copyMail' : 'modifyMail').'&amp;type='.$objFWUser->objMail->getType().'&amp;access_mail_lang=\'+(typeof(document.getElementsByName(\'access_mail_lang\')[0]) != \'undefined\' ? document.getElementsByName(\'access_mail_lang\')[0].value : 0)+\'&amp;access_change_format=1\';document.getElementById(\'access_mail_form\').submit()" size="1" style="width:400px;"'),
			'ACCESS_MAIL_SENDER_ADDRESS'	=> htmlentities($objFWUser->objMail->getSenderMail(), ENT_QUOTES, CONTREXX_CHARSET),
			'ACCESS_MAIL_SENDER_NAME'		=> htmlentities($objFWUser->objMail->getSenderName(), ENT_QUOTES, CONTREXX_CHARSET),
			'ACCESS_MAIL_BODY_TEXT'			=> htmlentities($objFWUser->objMail->getBodyText(), ENT_QUOTES, CONTREXX_CHARSET),
			'ACCESS_MAIL_BODY_HTML'			=> $objFWUser->objMail->getFormat() != 'text' ? get_wysiwyg_editor('access_mail_body_html', $objFWUser->objMail->getBodyHtml()) : '<input type="hidden" name="access_mail_body_html" value="'.htmlentities($objFWUser->objMail->getBodyHtml(), ENT_QUOTES, CONTREXX_CHARSET).'" />',
			'ACCESS_MAIL_HTML_BODY_STAUTS'	=> $objFWUser->objMail->getFormat() != 'text' ? 'block' : 'none',
			'ACCESS_MAIL_TEXT_BODY_STAUTS'	=> $objFWUser->objMail->getFormat() == 'text' ? 'block' : 'none',
			'ACCESS_MAIL_HTML_BODY_CLASS'	=> $objFWUser->objMail->getFormat() != 'text' ? 'active' : '',
			'ACCESS_MAIL_TEXT_BODY_CLASS'	=> $objFWUser->objMail->getFormat() == 'text' ? 'active' : '',
		));

		if ($objFWUser->objMail->getFormat() == 'text') {
		...
		} elseif ($objFWUser->objMail->getFormat() == 'html') {
		...
		$nr = 0;
		foreach ($objFWUser->objMail->getPlaceholders() as $placeholder => $placeholderTxt) {
			$this->_objTpl->setVariable(array(
	...
	function _configDeleteMail()
	{
		...
		$objFWUser = FWUser::getFWUserObject();
		if (!$objFWUser->objMail->load(contrexx_addslashes($_REQUEST['type']), !empty($_REQUEST['access_mail_lang']) ? intval($_REQUEST['access_mail_lang']) : null)) {
			return;
		}

		$objFWUser = FWUser::getFWUserObject();

		if ($objFWUser->objMail->delete()) {
