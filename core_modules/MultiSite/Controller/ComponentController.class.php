...
// TODO: implement multilanguage support for API command
    ...
    public function executeCommandWebsite($objTemplate, $arguments) {
        ...
        $websiteStateUrl = \Cx\Core\Routing\Url::fromMagic(ASCMS_PROTOCOL . '://' . $mainDomain . \Env::get('cx')->getBackendFolderName() . '/index.php?cmd=JsonData&object=MultiSite&act=updateOwnWebsiteState');
        $status = ($website->getStatus() == \Cx\Core_Modules\MultiSite\Model\Entity\Website::STATE_ONLINE);
        $objTemplate->setVariable(array(
            'MULTISITE_WEBSITE_STATUS_CHANGE_URL' => $websiteStateUrl->toString(),            
