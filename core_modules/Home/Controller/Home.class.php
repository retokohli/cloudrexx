    ...
    */
    ...
    function getHomePage()
    {
        ...
        if (/*!empty($arrItems) && */($_CONFIG['dashboardNews'] == 'on')) {
            ...
            $domain = $domainRepository->findOneBy(array('id' => 0));
            if ($domain) {
                $objTemplate->setGlobalVariable(array(
                    'CX_DOMAIN_URL' => ASCMS_PROTOCOL.'://'.$domain->getName(),
                    'CX_DOMAIN'     => $domain->getName(),
                ));
            }
            ...
            if ($websiteUser) {
                $objTemplate->setGlobalVariable(array(
                    'CX_SETUP_DATE' => date(ASCMS_DATE_FORMAT_DATE, $websiteUser->getRegistrationDate()),
                    'CX_CORE_MAIL'  => contrexx_raw2xhtml($websiteUser->getEmail()),
                ));
                if (!$websiteUser->isVerified()) {
                    $objTemplate->setVariable(array(
                        'CX_VERIFICATION_DATE' => date(ASCMS_DATE_FORMAT_DATE, $websiteUser->getRestoreKeyTime()),
                    ));
                    $objTemplate->parse('cx_account_verification');
                } else {
                    $objTemplate->hideBlock('cx_account_verification');
                }
            } else {
                $objTemplate->hideBlock('cx_account_verification');
            }
