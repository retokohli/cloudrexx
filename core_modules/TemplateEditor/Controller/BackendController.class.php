...
class BackendController extends SystemComponentBackendController {
    ...
    public function parsePage(\Cx\Core\Html\Sigma $template, array $cmd) {
        ...
        $fileStorage = new FileStorage($this->cx->getWebsiteThemesPath());
        $themeOptionRepository = new ThemeOptionsRepository($fileStorage);
        $themeRepository = new ThemeRepository();
        $themeID = isset($_GET['tid']) ? $_GET['tid'] : 1;
        $theme = $themeRepository->findById($themeID);
        $themeOptions = $themeOptionRepository->getByName($theme->getFoldername());
        $themeOptions->renderBackend($template);
        $template->setVariable('TEMPLATEEDITOR_IFRAME_URL', Url::fromModuleAndCmd('home','',null, array('preview' => $themeID, 'templateEditor' => 1)));
        $template->setVariable('TEMPLATEEDITOR_BACKURL', './index.php?cmd=ViewManager');
    }
