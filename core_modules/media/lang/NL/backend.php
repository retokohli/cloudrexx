<?php
/**
 * @copyright   CONTREXX CMS - COMVATION AG
 * @author      Comvation Development Team <info@comvation.com>
 * @access      public
 * @package     contrexx
 * @subpackage  core_module_media
 */
$_ARRAYLANG['TXT_PREVIEW'] = "Voorbeeld";
$_ARRAYLANG['TXT_MEDIA_FILE_NAME'] = "Bestandsnaam";
$_ARRAYLANG['TXT_MEDIA_FILE_SIZE'] = "Bestandsgrootte";
$_ARRAYLANG['TXT_MEDIA_FILE_DATE'] = "Datum";
$_ARRAYLANG['TXT_MEDIA_FILE_PERM'] = "Permissies";
$_ARRAYLANG['TXT_MEDIA_ARCHIVE'] = "Bestandsarchief";
$_ARRAYLANG['TXT_IMAGE_ADMINISTRATION'] = "Afbeeldingsbeheer";
$_ARRAYLANG['TXT_MEDIA_DIR_EMPTY'] = "Deze map is leeg!";
$_ARRAYLANG['TXT_MEDIA_NEW_DIRECTORY'] = "Nieuwe map aanmaken";
$_ARRAYLANG['TXT_MEDIA_CREATE'] = "Aanmaken";
$_ARRAYLANG['TXT_MEDIA_NAME'] = "Naam";
$_ARRAYLANG['TXT_MEDIA_CHECK_NAME'] = "Geef een bestandsnaam op!";
$_ARRAYLANG['TXT_MEDIA_FILE_TYPE'] = "Type";
$_ARRAYLANG['TXT_MEDIA_FILE_DIRECTORY'] = "Bestandsmap";
$_ARRAYLANG['TXT_MEDIA_FILE'] = "Bestand";
$_ARRAYLANG['TXT_MEDIA_UPLOAD_FILES'] = "Bestanden uploaden";
$_ARRAYLANG['TXT_MEDIA_UPLOAD'] = "Uploaden";
$_ARRAYLANG['TXT_MEDIA_MSG_ERROR_NEW_DIR'] = "Error: de nieuwe map kan niet worden aangemaakt! ";
$_ARRAYLANG['TXT_MEDIA_MSG_NEW_DIR'] = "De nieuwe map is succesvol aangemaakt!";
$_ARRAYLANG['TXT_MEDIA_MSG_NEW_FILE'] = "Bestanden zijn succesvol geupload!";
$_ARRAYLANG['TXT_MEDIA_MSG_ERROR_NEW_FILE'] = "Error: de bestanden kunnen niet geupload worden!";
$_ARRAYLANG['TXT_MEDIA_DELETE'] = "Verwijderen";
$_ARRAYLANG['TXT_MEDIA_CONFIRM_DELETE_2'] = "Deze bestanden verwijderen? Attentie: deze handeling kan niet ongedaan worden gemaakt!";
$_ARRAYLANG['TXT_MEDIA_MSG_DIR_DELETE'] = "Map is succesvol aangemaakt!";
$_ARRAYLANG['TXT_MEDIA_MSG_FILE_DELETE'] = "De bestanden zijn succesvol verwijderd!";
$_ARRAYLANG['TXT_MEDIA_MSG_ERROR_DIR'] = "Error: de map kan niet verwijderd worden! Zorg ervoor dat de betreffende map helemaal leeg is.";
$_ARRAYLANG['TXT_MEDIA_MSG_ERROR_FILE'] = "Error: de bestanden kunnen niet worden verwijderd!";
$_ARRAYLANG['TXT_MEDIA_EDIT'] = "Bewerken";
$_ARRAYLANG['TXT_MEDIA_SAVE'] = "Opslaan";
$_ARRAYLANG['TXT_MEDIA_OVERVIEW'] = "Bestandsoverzicht";
$_ARRAYLANG['TXT_MEDIA_MSG_SEVERAL_NEW_FILE'] = "Meerdere bestanden kunnen niet geupload worden!";
$_ARRAYLANG['TXT_MEDIA_SELECT_ACTION'] = "Selecteer actie:";
$_ARRAYLANG['TXT_DESIGN_FILES_ADMINISTRATION'] = "Beheer designbestanden";
$_ARRAYLANG['TXT_MEDIA_MAKE_SELECTION'] = "Maak een selectie!";
$_ARRAYLANG['TXT_MEDIA_EDIT_FILE'] = "Bestand bewerken";
$_ARRAYLANG['TXT_MEDIA_ERROR_OCCURED'] = "Er is een error opgetreden!";
$_ARRAYLANG['TXT_MEDIA_FILE_DONT_EXISTS'] = "Het gevraagde bestand bestaat niet!";
$_ARRAYLANG['TXT_MEDIA_BACK'] = "Terug";
$_ARRAYLANG['TXT_MEDIA_MSG_EDIT'] = "De bestanden zijn succesvol aangepast!";
$_ARRAYLANG['TXT_MEDIA_MSG_ERROR_EDIT'] = "Error: de bestanden kunnen niet worden aangepast!";
$_ARRAYLANG['TXT_MEDIA_RENAME_EXT'] = "De bestandsextentie wijzigen?";
$_ARRAYLANG['TXT_MEDIA_SELECT_UPLOAD_FILE'] = "Selecteer een bestand!";
$_ARRAYLANG['TXT_MEDIA_CUT'] = "Knippen";
$_ARRAYLANG['TXT_MEDIA_PASTE'] = "Plakken";
$_ARRAYLANG['TXT_MEDIA_MSG_CUT'] = "De bestanden zijn succesvol verplaatst!";
$_ARRAYLANG['TXT_MEDIA_COPY'] = "Kopiëren";
$_ARRAYLANG['TXT_MEDIA_MSG_COPY'] = "De bestanden zijn succesvol gekopieerd!";
$_ARRAYLANG['TXT_MEDIA_RENAME_NAME'] = "Geef een bestandsnaam op!";
$_ARRAYLANG['TXT_MEDIA_MSG_ERROR_COPY'] = "Error: de map kan niet in zichzelf gekopieerd worden!";
$_ARRAYLANG['TXT_MEDIA_MSG_ERROR_CUT'] = "Error: de map kan niet in zichzelf verplaatst worden!";
$_ARRAYLANG['TXT_MEDIA_WIDTH'] = "Wijdte";
$_ARRAYLANG['TXT_MEDIA_HEIGHT'] = "Hoogte";
$_ARRAYLANG['TXT_MEDIA_BALANCE'] = "Proporties behouden";
$_ARRAYLANG['TXT_MEDIA_RESET'] = "Terugzetten";
$_ARRAYLANG['TXT_MEDIA_QUALITY'] = "Kwaliteit";
$_ARRAYLANG['TXT_MEDIA_EDIT_IMAGE'] = "Afbeelding bewerken";
$_ARRAYLANG['TXT_MEDIA_NO'] = "Nee";
$_ARRAYLANG['TXT_MEDIA_YES'] = "Ja";
$_ARRAYLANG['TXT_MEDIA_INSERT_AS_COPY'] = "Als kopie opslaan";
$_ARRAYLANG['TXT_DESIGN_OVERVIEW'] = "Designoverzicht";
$_ARRAYLANG['TXT_NEW_DESIGN'] = "Nieuw design";
$_ARRAYLANG['TXT_ACTIVATE_DESIGN'] = "Activeren";
$_ARRAYLANG['TXT_DESIGN_REPLACEMENTS_DIR'] = "Vervangende map";
$_ARRAYLANG['TXT_MEDIA_MSG_FILENAME_REPLACED'] = "Some special characters ";
$_ARRAYLANG['TXT_DIR_META_KEYS'] = "Meta keys";
$_ARRAYLANG['TXT_IMAGE_SHOP'] = "Shop";
$_ARRAYLANG['TXT_IMAGE_CONTENT'] = "Content";
$_ARRAYLANG['TXT_MEDIA_FORCE_OVERWRITE'] = "Vorhandene Dateien überschreiben";
$_ARRAYLANG['TXT_MEDIA_RENAME'] = "Rename";
$_ARRAYLANG['TXT_MEDIA_SAVE_AS'] = "Save as";
$_ARRAYLANG['TXT_MEDIA_EDIT_FILE'] = "Rename media";
$_ARRAYLANG['TXT_MEDIA_IMAGE_MANIPULATION'] = "Image manipulation";
?>
