        ...
        case "shop":
             $query = "
                SELECT id, title, CONCAT_WS(' ', shortdesc, description) AS content,
                 ...
                 WHERE shortdesc LIKE '%".htmlentities($searchTerm, ENT_QUOTES, CONTREXX_CHARSET)."%'
                    OR description LIKE '%".htmlentities($searchTerm, ENT_QUOTES, CONTREXX_CHARSET)."%'
                    OR title LIKE '%$searchTerm%'
                   AND status=1
            ";
            break;
...
function search_getResultArray($query, $section_var, $cmd_var, $pagevar)
{
    ...
    if ($objResult !== false) {
        while (!$objResult->EOF) {
            ++$i;
            $cmd = (isset($objResult->fields['cmd']) ? $objResult->fields['cmd'] : '');
            $section = (isset($objResult->fields['section']) ? $objResult->fields['section'] : '');
            if ($section == '' && $section_var != '') $section = $section_var;
            if ($cmd == '' && $cmd_var != '') $cmd = $cmd_var;
