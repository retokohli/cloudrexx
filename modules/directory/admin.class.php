...
class rssDirectory extends directoryLibrary
{
    ...
    function __construct()
    {
        ...
        //check chmod
        $obj_file = new File();
        $obj_file->setChmod($this->mediaPath, $this->mediaWebPath, "");
    ...
    function showLevels()
    {
        ...
        $this->_objTpl->setVariable(array(
            ...
            'TXT_COLLAPSE_ALL'              => $_ARRAYLANG['TXT_DIR_COLLAPS'],
        ...
        //call collapse/expand/status function
        $this->expand();
        $this->collapse();
        $this->status();
            ...
            foreach ($this->levels['name'] as $levelKey => $levelName) {
                if ($this->levels['parentid'][$levelKey] == $parentId) {
                    ...
                    if ($_SESSION['expLevel'][$levelKey] == 1) {
                        $icon = "<a href='index.php?cmd=directory&amp;act=levels&amp;collapse=$levelKey'><img src='".$this->imageWebPath."directory/minuslink.gif' border='0' alt='' /></a>";
...
$padding = 0;
        ...
        foreach ($this->levels['name'] as $levelKey => $levelName) {
            if ($this->levels['parentid'][$levelKey] == $parentId) {
                if ($_SESSION['expLevel'][$parentId] == 1) {
                    ...
                    if ($_SESSION['expLevel'][$levelKey] == 1) {
                        $link = "<a href='index.php?cmd=directory&amp;act=levels&amp;collapse=$levelKey'><img src='".$this->imageWebPath."directory/minuslink.gif' border='0' alt='' /></a>";
                    } elseif (!in_array($levelKey, $this->levels['parentid'])) {
    ...
    function addLevel()
    {
        ...
        $objResult = $objDatabase->Execute("
            INSERT INTO ".DBPREFIX."module_directory_levels
               SET parentid=".$levLevel.",
                   name='".$levName."',
    ...
    function editLevel()
    {
        ...
        //get categorie data
        $objResult = $objDatabase->Execute("
            SELECT name, description, metakeys, metadesc,
                   parentid, showlevels, showcategories, onlyentries
        ...
        if ($levelOnlyEntries == '1') {
            $checkedEntries = 'checked="checked"';
        }
        if ($levelShowLevels == '1') {
            $checkedLevels = 'checked="checked"';
        }
        if ($levelShowCategories == '1') {
            $checkedCategories = 'checked="checked"';
        }
        if ($levelShowCategories == '1' && $levelShowLevels == '1') {
            $checkedCategories = '';
            $checkedLevels     = '';
            $checkedBoth       = 'checked="checked"';
        }
...
//            'CHECKED_NO'            => $checkedNo,
    ...
    function updateLevel()
    {
        ...
        if (isset($_POST['edit_submit'])) {
            ...
            // Update database
            $objResult = $objDatabase->Execute("
                UPDATE ".DBPREFIX."module_directory_levels
                   SET parentid=".$levParentId.",
                       name='".$levName."',
                       description='".$levDescription."',
                       status='1',
                       metadesc='".$levMetadesc."',
        ...
        if (in_array(0, $this->categories['parentid'])) {
            foreach ($this->categories['name'] as $catKey => $catName) {
                if ($this->categories['parentid'][$catKey] == $parentId) {
                    //set category icon
                    if (!empty($_SESSION['expCat'][$catKey])) {
                        $icon =
                            '<a href="index.php?cmd=directory&amp;collapse='.
                            $catKey.'"><img src="'.$this->imageWebPath.
                            'directory/minuslink.gif" border="0" alt="" /></a>';
                    } elseif (!in_array($catKey, $this->categories['parentid'])) {
                        $icon =
                            '<img src="'.$this->imageWebPath.
                            'directory/pixel.gif" width="11" height="1" border="0" alt="" />';
                    } else {
                        $icon =
                            '<a href="index.php?cmd=directory&amp;expand='.
                            $catKey.'"><img src="'.$this->imageWebPath.
                            'directory/pluslink.gif" border="0" alt="" /></a>';
                    }
                    //set folder image (active/inactive)
                    if ($this->categories['status'][$catKey] == 1) {
                        $folder =
                            '<a href="javascript:{statusCategory('.$catKey.
                            ',0);}"><img src="'.$this->imageWebPath.
                            'directory/_folder.gif" border="0" alt="" /></a>';
                    } else {
                        $folder =
                            '<a href="javascript:{statusCategory('.$catKey.
                            ',1);}"><img src="'.$this->imageWebPath.
                            'directory/_folder_off.gif" border="0" alt="" /></a>';
                    }
                    //set show categories (active/inactive)
...
$padding = 0;
    ...
    /**
     * Show subcategories of the category
     * @access   public
     * @param    string  $parentId
     * @param    string  $padding
     * @param    string  $i
    ...
    function showSubcategories($parentId, $padding, $i)
    {
        ...
        foreach ($this->categories['name'] as $catKey => $catName) {
            if ($this->categories['parentid'][$catKey] == $parentId) {
                if (!empty($_SESSION['expCat'][$parentId])) {
                    //set subcategorie icon
                    if (!empty($_SESSION['expCat'][$catKey])) {
                        $link = "<a href='index.php?cmd=directory&amp;collapse=$catKey'><img src='".$this->imageWebPath."directory/minuslink.gif' border='0' alt='' /></a>";
                    } elseif (!in_array($catKey, $this->categories['parentid'])) {
                        $link = "<img src='".$this->imageWebPath."directory/pixel.gif' width='11' height='1' border='0' alt='' />";
                    } else {
    ...
    function expand()
    {
        if (isset($_GET['expand'])) {
            if ($_GET['expand'] == "all") {
                if ($_GET['act'] == "levels") {
                    foreach (array_keys($this->levels['name']) as $levelKey) {
                        $_SESSION['expLevel'][$levelKey] = 1;
                    }
                } else {
                    foreach (array_keys($this->categories['name']) as $catKey) {
    ...
    function collapse()
    {
        if (isset($_GET['collapse'])) {
            if ($_GET['collapse'] == "all") {
                ...
                if ($_GET['act'] == "levels") {
                    $_SESSION['expLevel'][$_GET['collapse']] = "";
                } else {
                    $_SESSION['expCat'][$_GET['collapse']] = "";
                }
            }
        }
    ...
    function addCategorie()
    {
        ...
        //insert into database
        $objDatabase->Execute("
            INSERT INTO ".DBPREFIX."module_directory_categories
               SET parentid=".$catCategorie.",
                   name='".$catName."',
    ...
    function move()
    {
        ...
        {
            case'move':
                foreach ($_POST["formSelected"] as $catKey) {
                    $parentId = intval($_POST['selectCat']);
                    if ($parentId != $catKey) {
                        $objDatabase->Execute("UPDATE ".DBPREFIX."module_directory_categories SET parentid=".$parentId." WHERE id='".$catKey."'");
                    }
                }
            break;
            case'movelevel':
                foreach ($_POST["formSelected"] as $levelKey) {
                    $parentId = intval($_POST['selectLevel']);
                    if ($parentId != $levelKey) {
                        $objDatabase->Execute("UPDATE ".DBPREFIX."module_directory_levels SET parentid=".$parentId." WHERE id='".$levelKey."'");
                    }
                }
            break;
            case'movefile':
                ...
                if (isset($_POST['move_submit'])) {
                    foreach ($_SESSION['formSelected'] as $fileKey) {
                        //save categories
                        if (!empty($_POST["selectedCat"])) {
                            $objDatabase->Execute("DELETE FROM ".DBPREFIX."module_directory_rel_dir_cat WHERE dir_id='".$fileKey."'");

                            foreach ($_POST["selectedCat"] as $catKey) {
                        ...
                        if (!empty($_POST["selectedLevel"])) {
                            ...
                            foreach ($_POST["selectedLevel"] as $levelKey) {
    ...
    function delete()
    {
        switch ($_GET['act'])
        {
            case'delfile':
                if (!isset($_GET['id'])) {
                    foreach ($_POST["formSelected"] as $feedKey) {
...
//                $typ = $objResult->fields['typ'];
    ...
    function delLevel($levelId)
    {
        ...
        //del Level
        $objDatabase->Execute("DELETE FROM ".DBPREFIX."module_directory_levels WHERE id='$levelId'");
    ...
    function delSublevel($levelId)
    {
        ...
        //search sublevels
        if (!empty($sublevelId)) {
            foreach ($sublevelId as $sublevelId2) {
...
//                }
    ...
    function delCategorie($catId)
    {
        ...
        //del categorie
        $objDatabase->Execute("DELETE FROM ".DBPREFIX."module_directory_categories WHERE id='$catId'");
        $objDatabase->Execute("DELETE FROM ".DBPREFIX."module_directory_rel_dir_cat WHERE cat_id='".$catId."'");
        //del subcategories
    ...
    function status()
    {
        ...
        if (isset($_GET['status'])) {
            if ($_GET['act'] == "levels") {
                ...
                //change status
                $objDatabase->Execute("UPDATE ".DBPREFIX."module_directory_levels SET status='".$levelStatus."' WHERE id='".$levelId."'");
                ...
                //change status
                $objDatabase->Execute("UPDATE ".DBPREFIX."module_directory_categories SET status='".$catStatus."' WHERE id='".$catId."'");
    ...
    function catOrder()
    {
        ...
        {
            //update changes
            $objDatabase->Execute("UPDATE ".DBPREFIX."module_directory_categories SET displayorder=".intval($value)." WHERE id=".intval($catKey));
        }
        //status
...
//            'FILE'                        => $file,
    ...
    function showSettings_system()
    {
        ...
        $i = 0;
        $objResult = $objDatabase->Execute("
            SELECT setid, setname, setvalue, settyp
              FROM ".DBPREFIX."module_directory_settings
             WHERE settyp!='0'
               AND setid!='30'
               AND setname!='googlemap_start_location'
             ORDER BY settyp DESC
        ");
        if ($objResult) {
            while (!$objResult->EOF) {
                $allow_url_fopen = '';
                ...
                $this->_objTpl->setVariable(array(
                    'SETTINGS_ROWCLASS'    => (++$i % 2 ? 'row2' : 'row1'),
        ...
        $googleMapHTML .= $_ARRAYLANG['TXT_DIR_F_COUNTRY'].':</td><td><select style="width: 148px;" name="inputValue[country]">'.directoryLibrary::getCountryMenuOptions().'</select></td></tr></table><br />';
    ...
    function showSettings_google()
    {
        ...
        $objResult = $objDatabase->Execute("
            SELECT setid, setname, setvalue, settyp
              FROM ".DBPREFIX."module_directory_settings_google
             ORDER BY setid ASC
        ");
        if ($objResult) {
            while (!$objResult->EOF) {
                ...
                if ($objResult->fields['settyp'] == 1) {
                    $setValueField =
                        '<input type="text" name="setvalue['.
                        $objResult->fields['setid'].']" value="'.
                        $objResult->fields['setvalue'].'" size="90" maxlength="250" />';
                } elseif ($objResult->fields['settyp'] == 2) {
                    $true = '';
                    $false = '';
                    if ($objResult->fields['setvalue'] == 1) {
    ...
    function showSettings_inputs()
    {
        ...
        $objResult = $objDatabase->Execute("
            SELECT *
             ...
             ORDER BY active DESC, active_backend DESC, sort, typ, name
        ");
        if ($objResult) {
            while (!$objResult->EOF) {
                $checked  = '';
                ...
                if ($objResult->fields['name'] == 'title' || $objResult->fields['name'] == 'description') {
                    $setSearch = '<input type="checkbox" disabled="disabled" checked="checked" />';
                    $disabled  = 'disabled';
                }
                ...
                $setStatusField =
                    ...
                    $objResult->fields['id'].']" value="1" '.$disabled.' '.$checked.' />';
                ...
                if ($objResult->fields['typ'] == 3 || $objResult->fields['typ'] == 8 || $objResult->fields['typ'] == 9) {
                    $objResultDropdown = $objDatabase->Execute("
                        SELECT setid, setvalue
                          FROM ".DBPREFIX."module_directory_settings
                         WHERE setname='".$objResult->fields['name']."'
                    ");
                    if ($objResultDropdown && !$objResultDropdown->EOF) {
                        $textarea =
...
// TODO: This might overwrite the contents of $textarea...
...
// TODO: $class is not defined
                        //' class="'.$class.'"
                        ' style="background-color: '.$rowColor.';"><td>&nbsp;</td>'.
                        '<td valign="top"><div align="right"><i>Auswahlfelder:</i>&nbsp;</div></td>'.
                        '<td valign="top" colspan="7">'.$textarea.'</td></tr>';
...
// TODO: There should be one such record only, shouldn't it?
//                $objResult->MoveNext();
        }
...
//            $this->strOkMessage = $_ARRAYLANG['TXT_DIR_SETTINGS_SUCCESFULL_SAVE'];
        ...
        if (isset($_POST['set_inputs_submit'])) {
            ...
            if ($_POST['setSpezFields'] != "") {
                foreach ($_POST['setSpezFields'] as $id => $value) {
                    //update settings
                    $objDatabase->Execute("UPDATE ".DBPREFIX."module_directory_inputfields SET title='".contrexx_addslashes($value)."' WHERE id=".intval($id));
                }
            }
            //get dropdown data
        ...
        if (isset($_POST['inputValue']['zoom'])) {
            // The precision of the latitude and longitude values is
            // limited to at most six digits after the decimal point
            $arrMatch = array();
            if (preg_match('/^(\d?\d?\d?\d?\d?\d?)/', $_POST['inputValue']['lat_fraction'], $arrMatch)) {
                $latFraction = $arrMatch[1];
            }
            if (preg_match('/^(\d?\d?\d?\d?\d?\d?)/', $_POST['inputValue']['lon_fraction'], $arrMatch)) {
                $lonFraction = $arrMatch[1];
            }
            $googleStartPoint  =
                intval($_POST['inputValue']['lat']).'.'.$latFraction.
                ':'.
                intval($_POST['inputValue']['lon']).'.'.$lonFraction.
                ':'.
                intval($_POST['inputValue']['zoom']);
            $objDatabase->Execute("
                UPDATE ".DBPREFIX."module_directory_settings
                   SET setvalue='$googleStartPoint'
                 WHERE setname='googlemap_start_location'
            ");
        }
    }
