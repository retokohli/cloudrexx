...
class rssDirectory extends directoryLibrary
{
	...
	function showCategories($parentId)
	{
		...
		if(!empty($this->categories)){
			foreach($this->categories['name'] as $catKey => $catName){
				...
				$class= $parentId==0 ? "catLink" : "subcatLink";
				$this->arrRows[$i] .= "<a class='catLink' href='".CONTREXX_SCRIPT_PATH."?section=directory".$levelLink."&amp;cid=".$catKey."'>".htmlentities($catName, ENT_QUOTES, CONTREXX_CHARSET)."</a>&nbsp;(".$count.")<br />";
				...
				if($this->categories['parentid'][$catKey] == 0){
					...
					if($objResult !== false){
						while(!$objResult->EOF){
                            $this->arrRows[$i] .= "<a class='subcatLink' href='".CONTREXX_SCRIPT_PATH."?section=directory".$levelLink."&amp;cid=".$objResult->fields['id']."''>".htmlentities($objResult->fields['name'], ENT_QUOTES, CONTREXX_CHARSET)."</a>, ";
							$objResult->MoveNext();
						}
					}
    ...
    */
	function newFeed()
	{
		global $objDatabase, $_ARRAYLANG, $template, $_CONFIG;
	...
	*/
    function getVoting($id, $cid, $lid){
		...
		for ($x = 1; $x <= 10; $x++){
            $voteImg .= '<a href="'.CONTREXX_SCRIPT_PATH.'?section=directory&amp;cmd=vote&amp;id='.$id.'&amp;cid='.$cid.'&amp;lid='.$lid.'&amp;vote='.$x.'"><img src="'.$this->imageWebPath.'directory/'.$x.'.gif" border="0" alt="" /></a>&nbsp;';
		}
    ...
    */
	...
	function getNavtree($lid, $cid)
	{
		...
		foreach($this->navtreeLevels as $levelKey => $levelName){
            $navTreeLevel = "&nbsp;&raquo;&nbsp;<a href='".CONTREXX_SCRIPT_PATH."?section=directory&amp;lid=".$levelKey."'>".$levelName."</a>".$navTreeLevel;
		}
