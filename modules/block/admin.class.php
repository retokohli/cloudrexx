...
class blockManager extends blockLibrary
{
	...
	*/
	...
	{
		global $objTemplate, $_ARRAYLANG, $_CORELANG, $_CONFIG, $objDatabase;
	...
	function _showModifyBlock($copy = false)
	{
		...
		foreach ($arrLanguages as $langId => $arrLanguage) {
			...
			$this->_objTpl->setVariable(array(
				...
				'TXT_BLOCK_ACTIVATE'				=> "aktivieren",
				'TXT_BLOCK_SHOW_ON_ALL_PAGES'		=> "Block auf jeder Seite dieser Sprache anzeigen",
				'TXT_BLOCK_FRONTEND_PAGES'			=> "Frontend Seiten",
				'TXT_BLOCK_LANG_SHOW'				=> "Block in dieser Sprache anzeigen",
	...
	*/
	function _globalBlockOff()
	{
		...
		if($arrStatusBlocks != null){
			foreach ($arrStatusBlocks as $blockId){
				$query = "UPDATE ".DBPREFIX."module_block_blocks SET global='0' WHERE id=$blockId";
				$objDatabase->Execute($query);
			}
		}
	}
