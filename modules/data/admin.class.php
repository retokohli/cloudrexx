    ...
    */
    ...
    function showCategories() {
        ...
        if (count($arrCategories) > 0) {
            ...
            $this->parseCategoryDropdown($catTree, $arrCategories, 0, 0, FRONTEND_LANG_ID);
        } else {
    ...
    function parseCategoryLevel($tree, $arrCategories, $level, $intRowClass=1)
    {
        ...
        foreach ($tree as $key => $subcats) {
            ...
            $this->_objTpl->setVariable(array(
                ...
                'OVERVIEW_CATEGORY_NAME'        =>    $arrLanguages[FRONTEND_LANG_ID]['name'],
                ...
                'PARENT_OPT_LABEL'              =>  $arrLanguages[FRONTEND_LANG_ID]['name'],
    ...
    function editCategory($intCategoryId)
    {
        ...
        if (array_key_exists($intCategoryId,$arrCategories)) {
            ...
            $this->_objTpl->setVariable(array(
                ...
                'EDIT_NAME'              =>    $arrCategories[$intCategoryId][FRONTEND_LANG_ID]['name'],
    ...
    /**
     * Show an overview of all entries
     * @global    array
     */
    function showEntries()
    {
        ...
        $this->_objTpl->setVariable(array(
            ...
            'TITLE_CATEGORY'                    =>  ($intSelectedCategory == 0) ? $_ARRAYLANG['TXT_DATA_ALL'] : $arrCategories[$intSelectedCategory][FRONTEND_LANG_ID]['name'],
           ...
           if (count($this->arrEntries) > 0) {
               ...
               foreach ($this->arrEntries as $intEntryId => $arrEntryValues) {
                   if ($intSelectedCategory > 0) {
                       ...
                       if (!$this->categoryMatches($intSelectedCategory, $arrEntryValues['categories'][FRONTEND_LANG_ID])) {
                           continue;
                       }
                   }
                   $this->_objTpl->setVariable(array(
                ...
                $category_keys = array_keys($arrEntryValues['categories'][FRONTEND_LANG_ID]);
                if ($arrEntryValues['mode'] == "normal") {
                   ...
                   $this->_objTpl->setVariable(array(
                       ...
                       'ENTRY_CATEGORY'        =>  $arrCategories[$category_keys[0]][FRONTEND_LANG_ID]['name'],
                       'ENTRY_MODE'            =>  $mode
                   ));
                   if ($intSelectedCategory == 0) {
    ...
    function sortEntriesAlphabetical()
    {
        ...
        foreach ($this->arrEntries as $key => $entry ) {
            $assoc[$entry['translation'][FRONTEND_LANG_ID]['subject']] = $key;
        }
        ksort($assoc);
        $newEntries = array();
    ...
    {
        foreach ($categoryTree as $key => $value) {
            $fullName = $arrCategories[$key][FRONTEND_LANG_ID]['name'];
    ...
    function getAmountOfEntries($id)
    {
        ...
        foreach ($this->arrEntries as $entry) {
            if ($id == 0  || $this->categoryMatches($id, $entry['categories'][FRONTEND_LANG_ID])) {
                $amount++;
            }
        }
        return $amount;
    ...
    function insertEntryData($intMessageId)
    {
        ...
        foreach ($_POST as $strKey => $strValue) {
            if (substr($strKey,0,strlen('frmEditEntry_Subject_')) == 'frmEditEntry_Subject_') {
                ...
                $arrValues[$intLanguageId] = array(    'subject'         => contrexx_addslashes(strip_tags($strValue)),
                ...
                foreach ($arrEntryValues['categories'] as $intCategoryId) {
                    $objDatabase->Execute('    INSERT INTO '.DBPREFIX.'module_data_message_to_category
                                            SET `message_id` = '.$intMessageId.',
                                                ...
                                                `lang_id` = '.$intLanguageId.'
                                        ');
                    if ($intLanguageId == FRONTEND_LANG_ID) {
    ...
    function editEntry($intEntryId)
    {
        ...
        $catTree = $this->buildCatTree($arrCategories);
        $catKeys = array_keys($arrEntries[$intEntryId]['categories'][FRONTEND_LANG_ID]);
        $firstCat = $catKeys[0];
        $ie = (preg_match("/MSIE (6|7)/", $_SERVER['HTTP_USER_AGENT'])) ? true : false;
