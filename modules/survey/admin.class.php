 ...
 * survey module class
 *
 * @copyright	       CONTREXX CMS - Comvation Internet solutions
 * @author		Arunkumar.E - SS4U
 * @module		survey
 * @modulegroup	modules
 * @access		public
 * @version		1.0.0
...
class survey extends SurveyLibrary {
    ...
    function EditQuestionsOverview() {
        ...
        if ($objResult->RecordCount() == 0) {
            CSRF::header("Location: ".ASCMS_PATH_OFFSET.ASCMS_BACKEND_PATH."/index.php?cmd=survey&act=addQuestions&surveyId=".$id."&linkId=".$linkId);
        }
    ...
    function EditQuestions() {
        ...
        $link = 'index.php?cmd=survey&act=editQuestionsOverview&id='.$linkId.'&linkId='.$linkId.'&'.CSRF::param();
        ...
        if(isset($_POST['surveyQuestions_submit'])) {
            ...
            CSRF::header("Location: ".ASCMS_PATH_OFFSET.ASCMS_BACKEND_PATH."/index.php?cmd=survey&act=editQuestionsOverview&id=$survey_id&linkId=$linkId&mes=updated");
        }
    }
    ...
    function surveyOverview() {
        ...
        if ($objResult->RecordCount() == 0) {
            CSRF::header("Location: ".ASCMS_PATH_OFFSET.ASCMS_BACKEND_PATH."/index.php?cmd=survey&act=createOrCopy");
        }
        ...
        while(!$objResult->EOF) {
            ...
            if(trim($descVar) != "") {
                $descTemp = $Descshort.'..<a href="#" title="'.$descVar.'" class="tooltip"><img border="0" src="'.ASCMS_MODULE_IMAGE_WEB_PATH.'/survey/comment.gif"><a>';
            }
            else {
...
// TODO: Remove methode if obsolete
    function listOverview() {
        ...
        while(!$objResult->EOF) {
            ...
            if(strlen($listnameVar) > 20) {
                if($listnameVar != "") {
                    $listTemp = $listnameShot.'..<a href="#" title="'.htmlspecialchars($listnameVar).'" class="tooltip"><img border="0" src="'.ASCMS_MODULE_IMAGE_WEB_PATH.'/survey/comment.gif"><a>';
                }
            }else {
                if($listnameVar != "") {
    ...
    */
    ...
    function deleteQuestions() {
        ...
        CSRF::header("Location: ".ASCMS_PATH_OFFSET.ASCMS_BACKEND_PATH."/index.php?cmd=survey&act=editQuestionsOverview&id=$lid&linkId=$lid&mes=deleted");
    }
    ...
    function AddSurvey() {
        ...
        if(isset($_POST['survey_submit'])) {
            ...
            CSRF::header("Location: ".ASCMS_PATH_OFFSET.ASCMS_BACKEND_PATH."/index.php?cmd=survey&act=addQuestions&surveyId=$lastId");
        }
    }
    ...
    function addQuestions() {
        ...
        if(isset($_POST['surveyQuestions_submit'])) {
            ...
            if (isset($_REQUEST['linkId']) && ($_REQUEST['linkId'] != "")) {
                ...
                $link = "index.php?cmd=survey&act=editQuestionsOverview&id=".$ids."&linkId=".$ids."&".CSRF::param();
                $this->_objTpl->setVariable(array(
                ...
                $link = 'index.php?cmd=survey&act=editQuestionsOverview&id='.$surveyId.'&linkId='.$surveyId.'&'.CSRF::param();
                $this->_objTpl->setVariable(array(
                    'ADD_QUESTION_HERE' => '<a href="index.php?cmd=survey&act=addQuestions&surveyId='.$surveyId.'" title="'.$_ARRAYLANG['TXT_SURVEY_ADDQUESTION_TXT'].'">
                                           '.$_ARRAYLANG['TXT_SURVEY_ADDQUESTION_ANOTHER_TXT'].'</a>',
                    'TXT_DONE'          => '<input type="button" name="Done" value="'.$_ARRAYLANG['TXT_SURVEY_DONE_TXT'].'" onclick= "window.location=\''.$link.'\'" />',
    ...
    function Settings() {
        ...
        if(isset($_POST['settings_submit'])) {
            ...
            CSRF::header("Location: ".ASCMS_PATH_OFFSET.ASCMS_BACKEND_PATH."/index.php?cmd=survey&act=settings&mes=update");
        }
...
END;
    ...
    function csvSurvey() {
        ...
        if(!empty($id)) {
            ...
            CSRF::header("Content-Type: text/csv");
            CSRF::header("Content-Disposition: Attachment; filename=\"export.csv\"");
    ...
    function csvAdditionalinfo() {
        ...
        if(!empty($id)) {
            ...
            */
            ...
            CSRF::header("Content-Type: text/csv");
            CSRF::header("Content-Disposition: Attachment; filename=\"exportAdditionaldetails.csv\"");
