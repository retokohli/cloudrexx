...
class Payment
{
    ...
    */
    function getCountriesRelatedPaymentIdArray($countryId, $arrCurrencies)
    {
        ...
        $arrPaymentId=array();
        $query = "SELECT p.payment_id as payment_id
        ...
        while (!$objResult->EOF){
            ...
            && ($this->arrPaymentObject[$objResult->fields['payment_id']]['processor_id'] != 2 || count($arrAcceptedCurrencyCodes) > 0))
            {
    ...
    */
    function getPaymentMenu($selectedId=0, $onchange="", $countryId=0, $arrCurrencies='')
    {
        ...
        foreach($arrPaymentId as $id){
            ...
            $menu .="<option value=\"".$id."\" ".$selected.">".$this->arrPaymentObject[$id]['name']."</option>\n";
        }
        $menu .= "</select>\n";
        return $menu;
    }
     ...
     * @global  mixed   $objDatabase    Database object
     * @todo This method might well belong to the PaymentProcessing class.
     */
