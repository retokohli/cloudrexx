...
function safeNew($strClassName) {
    if(!class_exists($strClassName)) {
        throw new Exception($strClassName.' Class not exists!');
    }
            ...
            while (!$objInputfields->EOF) {
                ...
                if ($objInputfieldAttributes !== false) {
                    while (!$objInputfieldAttributes->EOF) {
                        ...
                        $arrInputfieldDefaultValue[$objInputfieldAttributes->fields['lang_id']] = htmlspecialchars($objInputfieldAttributes->fields['field_default_value'], ENT_QUOTES, CONTREXX_CHARSET);
                    ...
                    if($arrInputfield['id'] != 1 && $arrInputfield['id'] != 2) {
                        ...
                        foreach ($this->arrFrontendLanguages as $key => $arrLang) {
                            if(($key+1) == count($this->arrFrontendLanguages)) {
                                $minimize = "<a id=\"inputfieldMinimize_".$arrInputfield['id']."\" href=\"javascript:ExpandMinimizeInputfields('inputfieldName', '".$arrInputfield['id']."'); ExpandMinimizeInputfields('inputfieldDefaultvalue', '".$arrInputfield['id']."'); ExpandMinimizeInputfields('inputfieldLanguages', '".$arrInputfield['id']."');\">&laquo;&nbsp;".$_ARRAYLANG['TXT_MEDIADIR_MINIMIZE']."</a>";
                            } else {
                                $minimize = "";
                            }
    ...
    function saveInputfields($arrData)
    {
        ...
        foreach ($arrData['inputfieldId'] as $intKey => $intFieldId) {
            ...
            foreach ($this->arrFrontendLanguages as $key => $arrLang) {
                ...
                $strFieldName = contrexx_addslashes(contrexx_strip_tags($arrFieldNames[$arrLang['id']]));
                $strFieldDefaultValue = contrexx_addslashes(contrexx_strip_tags($arrFieldDefaultValues[$arrLang['id']]));
                ...
                if($arrLang['id'] == $_LANGID) {
                    if($this->arrInputfields[$intFieldId]['name'][0] == $arrFieldNames[0] && $this->arrInputfields[$intFieldId]['name'][$arrLang['id']] != $arrFieldNames[$arrLang['id']]) {
                        $strFieldName = contrexx_addslashes(contrexx_strip_tags($arrFieldNames[$_LANGID]));
                    }
                    ...
                    if($this->arrInputfields[$intFieldId]['default_value'][0] == $strFieldDefaultValue[0] && $this->arrInputfields[$intFieldId]['default_value'][$arrLang['id']] != $arrFieldDefaultValues[$arrLang['id']]) {
                        $strFieldDefaultValue = contrexx_addslashes(contrexx_strip_tags($arrFieldDefaultValues[$_LANGID]));
                    }
                    ...
                    if($this->arrInputfields[$intFieldId]['name'][0] != $arrFieldNames[0] && $this->arrInputfields[$intFieldId]['name'][$arrLang['id']] == $arrFieldNames[$arrLang['id']] ||
                       ...
                       $this->arrInputfields[$intFieldId]['name'][0] == $arrFieldNames[0] && $this->arrInputfields[$intFieldId]['name'][$arrLang['id']] == $arrFieldNames[$arrLang['id']]) {
                        $strFieldName = contrexx_addslashes(contrexx_strip_tags($arrFieldNames[0]));
                    }
                    ...
                    if($this->arrInputfields[$intFieldId]['default_value'][0] != $arrFieldDefaultValues[0] && $this->arrInputfields[$intFieldId]['default_value'][$arrLang['id']] == $arrFieldDefaultValues[$arrLang['id']] ||
                       ...
                       $this->arrInputfields[$intFieldId]['default_value'][0] == $arrFieldDefaultValues[0] && $this->arrInputfields[$intFieldId]['default_value'][$arrLang['id']] == $arrFieldDefaultValues[$arrLang['id']]) {
                       $strFieldDefaultValue = contrexx_addslashes(contrexx_strip_tags($arrFieldDefaultValues[0]));
                    }
                }

                if(empty($strFieldName)) $strFieldName = contrexx_addslashes(contrexx_strip_tags($arrFieldNames[0]));

                if(empty($arrFieldDefaultValues[0]) && $this->arrInputfields[$intFieldId]['default_value'][$arrLang['id']] == $arrFieldDefaultValues[$arrLang['id']]) {
                ...
                $objSaveInputfieldName = $objDatabase->Execute("
                    ...
                    SET
                        ...
                        `field_default_value` = '".$strFieldDefaultValue."'
                ");
