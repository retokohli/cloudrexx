...
class mediaDirectoryCategory extends mediaDirectoryLibrary
{
        ...
        if ($objCategories !== false) {
            while (!$objCategories->EOF) {
                ...
                if($this->arrSettings['settingsCountEntries'] == 1 || $objInit->mode == 'backend') {
                    $arrCategory['catNumEntries'] = $this->countEntries(intval($objCategories->fields['id']), intval($_GET['lid']));
                }
    ...
    function listCategories($objTpl, $intView, $intCategoryId=null, $arrParentIds=null, $intEntryId=null, $arrExistingBlocks=null)
    {
            ...
            case 1:
                ...
                foreach ($arrCategories as $key => $arrCategory) {
                    ...
                    if($_GET['exp_cat'] == 'all') {
                        ...
                        $bolExpandCategory = $this->getExpandedCategories($_GET['exp_cat'], array($arrCategory));
                    }
            ...
            case 4:
                ...
                foreach ($arrCategories as $key => $arrCategory) {
                    ...
                    if(in_array($arrCategory['catId'], $this->arrSelectedCategories)) {
                        $this->strSelectedOptions .= '<option value="'.$arrCategory['catId'].'">'.$spacer.$arrCategory['catName'][0].'</option>';
                    } else {
                        $this->strNotSelectedOptions .= '<option value="'.$arrCategory['catId'].'">'.$spacer.$arrCategory['catName'][0].'</option>';
                    }
