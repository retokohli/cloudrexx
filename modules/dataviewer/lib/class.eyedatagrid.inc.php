...
class EyeDataGrid {
	...
	*/
	private function buildFooter($shown, $first = 0, $last = 0)
	{
		...
		$xhtml .=  '
		<div class="dataviewerFooter">
			<div class="dataviewerFound">Found <em>' . $this->row_count . '</em> results';
		...
		if ($this->row_count > $this->results_per_page)
		{
			if ($this->page > 1)
				$xhtml .=  '
				<div class="dataviewerPagingLeft">
					<a href="javascript:;" onclick="tblSetPage(1)"><img src="' . $this->image_path . 'arrow_first.gif" alt="&lt;&lt;" title="First Page"></a><a href="javascript:;" onclick="tblSetPage(' . ($this->page - 1) . ')"><img src="' . $this->image_path . 'arrow_left.gif" alt="&lt;" title="Previous Page"></a>
				</div>';
			else
				$xhtml .=  '
				<div class="dataviewerPagingLeft">
					<img src="' . $this->image_path . 'arrow_first_disabled.gif" alt="arrow_first_disabled" title="First Page" /><img src="' . $this->image_path . 'arrow_left_disabled.gif" alt="previousPage" title="Previous Page" />
				</div>';
		...
		$xhtml .= '<div class="dataviewerPagingPages">';
      ...
      for ($i = $startpage; $i <= $endpage; $i++)
			{
				if ($i == $this->page)
					...
					$xhtml .=  '
						&nbsp;<a href="javascript:;" onclick="tblSetPage(' . $i . ')">' . $i . '</a>&nbsp;';
			}
		$xhtml .= "</div>";

			if ($this->page < $pages)
				$xhtml .=  '
				<div class="dataviewerPagingRight">
					<a href="javascript:;" onclick="tblSetPage(' . ($this->page + 1) . ')"><img src="' . $this->image_path . 'arrow_right.gif" alt="arrowRight" title="Next Page"></a><a href="javascript:;" onclick="tblSetPage(' . $pages . ')"><img src="' . $this->image_path . 'arrow_last.gif" alt="arrowlast" title="Last Page"></a>
				</div>';
			else
				$xhtml .=  '
				<div class="dataviewerPagingRight">
					<img src="' . $this->image_path . 'arrow_right_disabled.gif" alt="nextPAge" title="Next Page"><img src="' . $this->image_path . 'arrow_last_disabled.gif" alt="&gt;&gt;" title="Last Page">
				</div>';
		}

		$xhtml .=  '
			</div>
			<div class="dataviewerPagingDropDown">';
		...
		if ($pages > 0)
		{
			...
			if (!$this->hide_page_list and $pages > 1)
			{
				...
				$xhtml .=  '<select name="tbl-page" class="pageDD" onchange="tblSetPage(this.options[this.selectedIndex].value)">';
				for ($x = 1; $x <= $pages; $x++)
				{
...
//		echo $query . ' ' . $order . ' ' . $limit;
		...
		if ($this->create_button)
			$xhtml .=  '<span><a ' . $this->create_button['Action'] . ' title="' . $this->create_button['Text'] . '"><img src="' . $this->image_path . $this->img_create . '">' . $this->create_button['Text'] . '</a></span>';

		// Output the reset button
		if ($this->reset_button)
			$xhtml .=  '<span><a href="javascript:;" onclick="tblReset()" title="' . $this->reset_button .'"><img src="' . $this->image_path . $this->img_reset . '">' . $this->reset_button .'</a></span>';
		...
		if ($this->row_count == 0)
			$xhtml .=  '<tr><td colspan="' . $this->column_count . '">' . self::TXT_NORESULTS . '</td></tr>';
		else {
			$i = 0; $first = 0; $last = 0;
			...
			while ($row = $this->_db->fetchAssoc($this->result))
			{
				...
				if ($this->show_checkboxes)
					$xhtml .=  '<td align="center"><input type="checkbox" id="checkbox" name="tbl-checkbox" value="' . $row[$this->primary] . '"></td>';
