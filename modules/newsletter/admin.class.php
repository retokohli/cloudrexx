...
//                }
    ...
    {
        global $objDatabase, $_ARRAYLANG;
        $this->_pageTitle = $_ARRAYLANG['TXT_NEWSLETTER_INTERFACE'];
...
// TODO: if useVirtualLanguagePath is not turned on, we shall add the url modificator langId to the profile URI
    ...
    function _getNewsPreviewPage()
    {
		...
		{
			$selectedNews = isset($_POST['selected']) ? $_POST['selected'] : '';
...
// TODO: Refactor this method
        ...
        if (isset($_POST['import_cancel'])) {
            ...
            if (isset($_POST['newsletter_import_plain'])) {
                if ($_REQUEST['category'] == 'selectcategory') {
                    ...
                    self::$strOkMessage = $_ARRAYLANG['TXT_DATA_IMPORT_SUCCESSFUL']."<br/>".$_ARRAYLANG['TXT_CORRECT_EMAILS'].": ".$EmailCount."<br/>".$_ARRAYLANG['TXT_NOT_VALID_EMAILS'].": &quote;".implode(', ', $arrBadEmails)."&quote;<br/>".$_ARRAYLANG['TXT_EXISTING_EMAILS'].": ".$ExistEmails."<br/>".$_ARRAYLANG['TXT_NEW_ADDED_EMAILS'].": ".$NewEmails;
                }
            }
    ...
    function _editUser()
    {
        ...
        $strLanguage = '<select name="language" style="width:296px">';
        ...
        while (!$objLanguage->EOF) {
            ...
            $strLanguage .= '<option value="'.$objLanguage->fields['id'].'" '.$selected.'>'.contrexx_raw2xhtml($objLanguage->fields['name']).'</option>';
