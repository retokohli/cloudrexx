 ...
 * @subpackage  module_immo
 * @version		1.0.0
 * @todo        extend the module_immo_settings table for even more customization, remove hardcoded stuff
 */
...
class Immo extends ImmoLib
{
	...
	* PHP5 constructor
	*
	* @global object $objDatabase ADODB abstraction layer object
	* @param $pageContent HTML template (@see /index.php)
	*/
	function __construct($pageContent)
	{
		...
		$objRS=$objDatabase->Execute("	SELECT count(1) as cnt FROM ".DBPREFIX."module_immo_fieldname WHERE
										lang_id = 1 AND lower(name) LIKE '%aufz�hlung%'");
	...
	*/
	...
	function _getListing(){
		for($i=1; $i<=$this->_listingCount;$i++){
			...
			if(!empty($list)){
				$this->_objTpl->setVariable('IMMO_LISTING', $this->_getFieldFromText('Aufz�hlung'.$i));
	...
	function _showObject(){
		...
		if(!empty($_GET['id'])){
			$immoID = intval($_GET['id']);
			if(empty($immoID)){
				header('Location: ?section=immo&cmd=immolist');
				die();
			}
		}
		...
		$img = $this->_getFieldFromText('�bersichtsbild', 'img');
	...
	function _showMap() {
	    ...
	    */
        $subQueryPart = "";
        $first = true;
        preg_match_all("/%([A-Z0-9���]+[^%])%/", $this->arrSettings['message'], $matches);
