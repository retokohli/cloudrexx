...
class Calendar extends calendarLibrary
{
     ...
     * Get Calendar Page
     *
     * Standard function, called by the index file
     * @access  public
     */
            ...
            case 'event':
                ...
                if (isset($_REQUEST['export'])) {
                    switch ($_REQUEST['export']) {
                        case 'iCal':
                            if ($key > 0) $this->_iCalExport('event', $key);
                            break;
                        case 'category':
                            if ($key > 0) $this->_iCalExport('category', $key);
                            break;
                        case 'all':
    ...
    function _getEventList()
    {
        ...
        if (empty($_POST['startDate'])) {
            ...
            if (   $_GET['cmd'] == 'boxes'
                && (empty($_GET['act']) || empty($_REQUEST['dayID'])))
                $day = 1;
    ...
    */
    function _showRegistrationForm()
    {
        ...
        if (!empty($_POST['id'])) {
            ...
            $host     = "";
            $query = "
                INSERT INTO ".DBPREFIX."module_calendar".$this->mandateLink."_registrations (
                    `note_id`, `note_date`, `time`,
                    `host`, `ip_address`, `type`
                ) VALUES (
                    '$noteId', '$noteDate', '$time',
                    '$host', '$ip', '$type'
                )";
            ...
            if ($objResultReg) {
                ...
                foreach ($_POST['signForm'] as $fieldId => $fieldData) {
                    ...
                    $query = "
                        INSERT INTO ".DBPREFIX."module_calendar".$this->mandateLink."_form_data (
                            `reg_id`, `field_id`, `data`
                        ) VALUES (
                            '$regId', '$fieldId', '$fieldData'
                        )";
...
//                $eventId = $this->objEvent->insert($eventValues);
                $this->objEvent->insert($eventValues);
