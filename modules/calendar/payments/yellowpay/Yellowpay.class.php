...
//        ),
    ...
    {
        self::$_objSettings = new CalendarSettings();
        /*$strAcceptedPaymentMethods =
            $objSettings->getYellowpaySettings('paymentYellowpayMethods');

        $strAcceptedPaymentMethods = str_replace(array("0", "1", "2", "3", "4", "5"), array("PostFinanceCard","yellownet","Master","Visa","Amex","Diners"), $strAcceptedPaymentMethods);
        */// There needs to be at least one accepted payment method,
        // if there is none, accept all.
        //die($strAcceptedPaymentMethods);
        $strAcceptedPaymentMethods = "PostFinanceCard,yellownet,Master,Visa,Amex,Diners";
    ...
    static function getForm(
        $arrOrder, $submitValue='send', $autopost=true
    ) {
        global $_ARRAYLANG;
        ...
        $match = array();
        if (preg_match('/^(.+section=calendar)/', $base_uri, $match)) {
            $base_uri = $match[1];
        } else {
            self::$arrError[] = 'Failed to determine base URI: '.$base_uri;
            return '';
        }
        $base_uri = $base_uri.'&cmd=success&handler=yellowpay&orderId='.self::$arrOrder["orderID"].'&result=';
        if (empty(self::$arrOrder['accepturl'])) {
            self::$arrOrder['accepturl'] = $base_uri.'1';
        }
        if (empty(self::$arrOrder['declineurl'])) {
            self::$arrOrder['declineurl'] = $base_uri.'2';
        }
        if (empty(self::$arrOrder['exceptionurl'])) {
            self::$arrOrder['exceptionurl'] = $base_uri.'2';
        }
        if (empty(self::$arrOrder['cancelurl'])) {
            self::$arrOrder['cancelurl'] = $base_uri.'0';
        }
        self::$form =
...
// CURRENT Postfinance E-Commerce URI
            ...
            (self::$_objSettings->getYellowpaySettings('paymentTestserver') == '1' ? 'test' : 'prod').
            '/orderstandard.asp"'.
            ">\n";
/*
            // Yellowpay dummy
...
//self::$arrError[] = "Test for error handling";
    ...
    {
        $seed = self::$_objSettings->getYellowpaySettings("paymentYellowpayShaIn");
        if (   empty(self::$arrOrder['orderID'])
            || empty(self::$arrOrder['amount'])
            || empty(self::$arrOrder['currency'])
            || empty(self::$arrOrder['PSPID'])
            || empty($seed)) {
            self::$arrError[] = 'Missing mandatory parameter for computing the hash';
            return false;
        }
        $hash_string =
            self::$arrOrder['orderID'].
            self::$arrOrder['amount'].
            self::$arrOrder['currency'].
            self::$arrOrder['PSPID'].
            $seed;
        self::$arrOrder['SHASign'] = sha1($hash_string);
        return true;
    }
    ...
    {
        foreach (array_keys(self::$arrOrder) as $key) {
    ...
    {
        if (!array_key_exists($key, self::$arrOrder)) {
            self::$arrError[] = "Missing key '$key'!";
            return false;
        }
        $value = self::$arrOrder[$key];
        // This one *MUST NOT* be used a second time
        unset(self::$arrOrder[$key]);
        switch ($key) {
            // Mandatory
            case 'orderID':
...
// *SHOULD* verify the URIs, but the expression is not fit
    ...
    static function checkHash()
    {
        ...
        $hash_string .= self::$_objSettings->getYellowpaySettings('paymentYellowpayShaOut');
    ...
    static function getAuthorizationMenuoptions()
    {
        ...
        self::$strAuthorization =
            self::$_objSettings->getYellowpaySettings('paymentYellowpayAuthorization');
        return
            '<option value="SAL"'.
