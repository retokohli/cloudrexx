...
class calendarLibrary
{
    ...
    function __construct($url)
    {
        ...
        $this->_objTpl = new HTML_Template_Sigma(ASCMS_MODULE_PATH.'/calendar'.MODULE_INDEX.'/template');
        $this->_objTpl->setErrorHandling(PEAR_ERROR_DIE);
        $this->_objTpl->setGlobalVariable("MODULE_INDEX", MODULE_INDEX);
        $this->url = $url.MODULE_INDEX;
    }
...
//        $this->_filename = html_entity_decode($objRS->fields['name'], ENT_QUOTES, CONTREXX_CHARSET);
    ...
    function getEventsFromCategoryByID($categoryID)
    {
        ...
        $query = "  SELECT `id` FROM `".DBPREFIX."module_calendar".MODULE_INDEX."`
     ...
     * @return  mixed               The category name on success,
     *                              false otherwise
     * @static
     * @author  reto.kohli@comvation.com
     */
    static function getCategoryNameByCategoryId($catId)
    {
        global $objDatabase, $_ARRAYLANG;
    ...
    function getDayNote($id, $showboxes=true)
    {
        ...
        $this->_objTpl->setVariable(array(
            ...
            'CALENDAR_ICAL_EXPORT'          => '<a href="'.CONTREXX_DIRECTORY_INDEX.'?section=calendar'.MODULE_INDEX.'&amp;cmd=event&amp;export=iCal&amp;id='.$id.'" title="'.$_ARRAYLANG['TXT_CALENDAR_EXPORT_ICAL_EVENT'].'">
            ...
            'CALENDAR_ICAL_EXPORT_IMG'      => '<a href="'.CONTREXX_DIRECTORY_INDEX.'?section=calendar'.MODULE_INDEX.'&amp;cmd=event&amp;export=iCal&amp;id='.$id.'" title="'.$_ARRAYLANG['TXT_CALENDAR_EXPORT_ICAL_EVENT'].'">
...
//        $end_year        = date("Y", $enddate);
        ...
        $this->_objTpl->setVariable(array(
            ...
            'CALENDAR_REGISTRATION_LINK'                    => '<a href="'.CONTREXX_DIRECTORY_INDEX.'?section=calendar'.MODULE_INDEX.'&amp;cmd=sign&amp;id='.$objResultNote->fields['id'].'">'.$_ARRAYLANG['TXT_CALENDAR_REGISTRATION_LINK'].'</a>',
            ...
            if ($objResultField !== false) {
                while (!$objResultField->EOF) {
                    ...
                    switch ($objResultField->fields['type']) {
                        case 1:
                            if($objResultField->fields['key'] == 13) {
                                for ($i=1; $i <= 99; $i++) {
                                    $options .= '<option value="'.$i.'">'.$i.'</option>';
                                }
                                $inputField = '<select name="signForm['.$objResultField->fields['id'].']" style="width: 254px;" /><option value="0">'.$_ARRAYLANG['TXT_CALENDAR_REG_NO_ESCORT'].'</option>'.$options.'</select>';
                            } else {
    ...
    function _sendRegistration($noteId)
    {
        ...
        if (@include_once ASCMS_LIBRARY_PATH.'/phpmailer/class.phpmailer.php') {
            ...
            foreach ($arrUsers as $userId => $arrUser) {
                ...
                $link       = "http://".$url."/".($_CONFIG['useVirtualLanguagePath'] == 'on' ? $objLanguage->getLanguageParameter($languageId, 'lang').'/' : null).CONTREXX_DIRECTORY_INDEX."?section=calendar".MODULE_INDEX."&cmd=sign&key=".$key;
    ...
    function _sendConfirmation($user, $noteId, $regId)
    {
        ...
        if (@include_once ASCMS_LIBRARY_PATH.'/phpmailer/class.phpmailer.php') {
            ...
            $objMail->Send();
//echo("_sendConfirmation($user, $noteId, $regId): Mail sent: ".htmlentities(var_export($objMail, true))."<br />");
            $objMail->ClearAddresses();
        }
    }
    ...
    function _sendNotification($mail, $firstname, $lastname, $noteId, $regId)
    {
        ...
        if (@include_once ASCMS_LIBRARY_PATH.'/phpmailer/class.phpmailer.php') {
            if ($objResultNote->fields['notification'] == 1) {
                ...
                $objMail->Send();
//echo("_sendNotification($mail, $firstname, $lastname, $noteId, $regId): Mail sent: ".htmlentities(var_export($objMail, true))."<br />");
                $objMail->ClearAddresses();
            }
        }
    }
