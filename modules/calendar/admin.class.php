...
class calendarManager extends calendarLibrary
{
    ...
    function __construct()
    {
        ...
        parent::__construct($_SERVER["SCRIPT_NAME"]."?cmd=calendar".MODULE_INDEX);
        // links
        $this->pageTitle = $_ARRAYLANG['TXT_CALENDAR'];
        $objTemplate->setVariable("CONTENT_NAVIGATION","
            <a href='?cmd=calendar".MODULE_INDEX."'> ".$_ARRAYLANG['TXT_CALENDAR_MENU_OVERVIEW']." </a>
            <a href='?cmd=calendar".MODULE_INDEX."&amp;act=new'> ".$_ARRAYLANG['TXT_CALENDAR_MENU_ENTRY']." </a>
            <a href='?cmd=calendar".MODULE_INDEX."&amp;act=cat'> ".$_ARRAYLANG['TXT_CALENDAR_CATEGORIES']." </a>
            <a href='?cmd=calendar".MODULE_INDEX."&amp;act=placeholder'>".$_ARRAYLANG['TXT_CALENDAR_PLACEHOLDER']."</a>
            <a href='?cmd=calendar".MODULE_INDEX."&amp;act=settings'> ".$_ARRAYLANG['TXT_CALENDAR_MENU_SETTINGS']." </a>");
    ...
    function getCalendarPage()
    {
        ...
        switch ($_REQUEST['act']) {
            ...
            case 'saveNew':
                $id = $this->writeNote('');
                header("Location: ?cmd=calendar".MODULE_INDEX."&act=event&id=$id");
                exit;
            case 'getCSV':
                $id = intval($_GET['id']);
                $this->_getCsv($id);
            ...
            case 'saveEdited':
                ...
                header("Location: ?cmd=calendar".MODULE_INDEX);
                exit;
            case 'settings':
            ...
            case 'saveSettings':
                $this->saveSettings();
                $this->setStdCat();
                header("Location: ?cmd=calendar".MODULE_INDEX."&act=settings");
                exit;
            case 'event_actions':
                $this->multiDelete();
                break;
...
// TODO: Neither $arrFields nor $fieldIdSecond is defined
//            'TXT_CALENDAR_SECOND'         => $arrFields[$fieldIdSecond],
    ...
    function showRegistrationDetail($id)
    {
        ...
        $noteId = calendarLibrary::getNoteIdByRegistrationId($id);
        if ($noteId) {
            $this->_objTpl->setVariable(
    ...
    function multiDelete()
    {
        ...
        if (!empty($_POST['selectedEventId'])) {
            foreach ($_POST['selectedEventId'] as $eventid) {
                $query = "DELETE FROM ".DBPREFIX."module_calendar".MODULE_INDEX." WHERE id=$eventid";
        ...
        if ($this->mandate == 1) {
            ...
            header("LOCATION: ?cmd=calendar".MODULE_INDEX."".$this->mandate);
        }
        exit;
    }
    ...
    function delNote($id)
    {
        ...
        $objResultDel = $objDatabase->Execute("DELETE FROM ".DBPREFIX."module_calendar".MODULE_INDEX." WHERE id = ".intval($id));
...
//            $ed = get_wysiwyg_editor('inputComment');
            ...
            $query             = "SELECT setvalue
                                    FROM ".DBPREFIX."module_calendar".MODULE_INDEX."_settings
                                WHERE setid = '1'";
    ...
    function writeNote()
    {
        ...
        if (!empty($id)) {
            ...
            if ($objDatabase->Affected_Rows() > 0) {
                ...
                if ($objResult !== false) {
                    if ($registration == 1) {
                        $query = "DELETE FROM ".DBPREFIX."module_calendar".MODULE_INDEX."_form_fields WHERE note_id='".$id."'";
                        ...
                        if ($objResultFields !== false) {
                            ...
                            foreach (array_keys($registrationArrFieldStatus) as $fieldKey) {
                                ...
                                $query = "
                                    INSERT INTO ".DBPREFIX."module_calendar".MODULE_INDEX."_form_fields (
                                        `id`, `note_id`, `name`, `type`, `required`, `order`, `key`
                                    ) VALUES (
                                        $fieldId, '$id', '".addslashes($fieldName)."', '$fieldType',
                                        '$fieldRequired', '$fieldOrder', '$fieldKey')";
                                $objResultFields = $objDatabase->Execute($query);
                            }
                        }
            ...
            $query = "
                INSERT INTO ".DBPREFIX."module_calendar".MODULE_INDEX." (
                    ...
                    '$active', '$catid', '$startdate', '$enddate',
                    '$priority', '$access', '$title', '$comment',
                    '$link', '$pic', '$attach', '$place',
                    '$placeStreet', '$placeZip', '$placeCity', '$placeLink',
                    '$placeMap', '$organizer', '$organizerStreet', '$organizerZip',
                    '$organizerPlace', '$organizerMail', '$organizerLink', '$registration',
                    '$groups', '$all_groups', '$public', '$mailTitle',
                    '$mailContent', '$key', '$registrationSubscriber', '$notification',
                    '$notificationAddress'
                )
            ";
            $objResult = $objDatabase->Execute($query);
    ...
    function showCategories()
    {
        ...
        if (isset($_GET['new']) and $_GET['new'] == 1) {
            if ($_POST['name'] != '' and $_POST['status'] != '' and $_POST['lang']) {
                ...
                if ($objDatabase->Affected_Rows() == 0) {
                    $query = "INSERT INTO ".DBPREFIX."module_calendar".MODULE_INDEX."_categories
                                        SET name = '".$_POST['name']."',
        ...
        if (isset($_GET['chg']) and $_GET['chg'] == 1 and isset($_POST['selected']) and is_array($_POST['selected'])) {
            ...
            if ($_POST['form_delete'] != '') {
                ...
                foreach($ids as $id) {
                    ...
                    if ($objDatabase->Affected_Rows() == 0) {
                        $query = "DELETE FROM ".DBPREFIX."module_calendar".MODULE_INDEX."_categories
                                          WHERE id = '".intval($id)."'";
                ...
                $query = "SELECT id FROM ".DBPREFIX."module_calendar".MODULE_INDEX."_categories";
            ...
            if ($_POST['form_activate'] != '' or $_POST['form_deactivate'] != '') {
                ...
                foreach($ids as $id) {
                    $query = "UPDATE ".DBPREFIX."module_calendar".MODULE_INDEX."_categories
                                   SET status = '".$to."'
                                 WHERE id = '".intval($id)."'";
                    $objDatabase->SelectLimit($query, 1);
                }
        ...
        //table
        $query = " SELECT * FROM ".DBPREFIX."module_calendar".MODULE_INDEX."_categories
    ...
    function categoriesEdit()
    {
        ...
        if (isset($_GET['id']) and $_GET['id'] != '') {
            ...
            if ($objDatabase->Affected_Rows() == 0) {
                header("Location: index.php?cmd=calendar".MODULE_INDEX."&act=cat");
...
//        }
        ...
        $query = "
            SELECT * FROM ".DBPREFIX."module_calendarMODULE_INDEX
        ...
        while (!$objResult->EOF) {
            ...
            $this->_objTpl->setVariable(array(
                ...
                'CALENDAR_EVENT_CAT'       => calendarLibrary::getCategoryNameByCategoryId($objResult->fields['catid']),
            ));
            $this->_objTpl->parse("event");
            $objResult->moveNext();
        }
    ...
    function settings()
    {
        ...
        while (!$objResult->EOF) {
            if ($objResult->fields['id'] == $_CONFIG['calendar'.MODULE_INDEX.'headlinescat']) {
                $selected = " selected=\"selected\"";
            } else {
                $selected = "";
            }
        ...
        $this->_objTpl->setVariable(array(
            ...
            'CALENDAR_HEADLINESCOUNT'            => $_CONFIG['calendar'.MODULE_INDEX.'headlinescount'],
            ...
            'CALENDAR_STANDARDNUMBER'            => $_CONFIG['calendar'.MODULE_INDEX.'defaultcount'],
            'TXT_CALENDAR_MAIL_TEMPLATE'        => $_ARRAYLANG['TXT_CALENDAR_MAIL_TEMPLATE'],
            'TXT_CALENDAR_TITLE'                => $_ARRAYLANG['TXT_CALENDAR_TITLE'],
            'TXT_CALENDAR_TEXT'                    => $_ARRAYLANG['TXT_CALENDAR_TEXT'],
            'CALENDAR_MAIL_TITLE'                => $mailTitle,
...
//        set standard
        $query         = "SELECT stdCat
                           FROM ".DBPREFIX."module_calendar".MODULE_INDEX."_style
                        WHERE id = '2'";
    ...
    function setStdCat()
    {
        ...
        $query = "
            UPDATE ".DBPREFIX."module_calendar".MODULE_INDEX."_style
            SET stdCat = '".$string."'
            WHERE id = '2'";
    ...
    function saveSettings()
    {
        ...
        $query = "SELECT setvalue
                  ...
                  WHERE setname = 'calendar".MODULE_INDEX."headlines'";
        $objRs = $objDatabase->Execute($query);
        if ($objRs->RecordCount() == 0) {
            $query = "INSERT INTO ".DBPREFIX."settings
                      (setname, setvalue)
                      VALUES
                      ('calendar".MODULE_INDEX."headlines',
                       '".$val."')";
        } else {
        ...
        $query = "SELECT setvalue
                  FROM ".DBPREFIX."settings
                  WHERE setname = 'calendar".MODULE_INDEX."headlinescount'";
        $objRs = $objDatabase->Execute($query);
        if ($objRs->RecordCount() == 0) {
            $query = "INSERT INTO ".DBPREFIX."settings
                      (setname, setvalue)
                      VALUES
                      ('calendar".MODULE_INDEX."headlinescount',
        ...
        $query = "SELECT setvalue
                  FROM ".DBPREFIX."settings
                  WHERE setname = 'calendar".MODULE_INDEX."headlinescat'";
        $objRs = $objDatabase->Execute($query);
        if ($objRs->RecordCount() == 0) {
            $query = "INSERT INTO ".DBPREFIX."settings
                      (setname, setvalue)
                      VALUES
                      ('calendar".MODULE_INDEX."headlinescat',
        ...
        $query = "SELECT setvalue
                  FROM ".DBPREFIX."settings
                  WHERE setname = 'calendar".MODULE_INDEX."defaultcount'";
        $objRs = $objDatabase->Execute($query);
        if ($objRs->RecordCount() == 0) {
            $query = "INSERT INTO ".DBPREFIX."settings
                      (setname, setvalue)
                      VALUES
                      ('calendar".MODULE_INDEX."defaultcount',
        ...
        $query = "UPDATE ".DBPREFIX."module_calendar".MODULE_INDEX."_settings
    ...
    function activateNote()
    {
        ...
        if ($arrStatusNote != null) {
            foreach ($arrStatusNote as $noteId) {
                $query = "UPDATE ".DBPREFIX."module_calendar".MODULE_INDEX." SET active='1' WHERE id=$noteId";
                $objDatabase->Execute($query);
            }
        }
    }
    ...
    */
    function deactivateNote()
    {
        ...
        if ($arrStatusNote != null) {
            foreach ($arrStatusNote as $noteId) {
                $query = "UPDATE ".DBPREFIX."module_calendar".MODULE_INDEX." SET active='0' WHERE id=$noteId";
                $objDatabase->Execute($query);
            }
        }
    }
    ...
    function signRegistration($type)
    {
        ...
        foreach ($_POST['selectedRegId'] as $arrId) {
            $query = "UPDATE ".DBPREFIX."module_calendar".MODULE_INDEX."_registrations SET type=$type WHERE id=$arrId";
            $objDatabase->Execute($query);
        }
    }
    ...
    function deleteRegistration($regId)
    {
        ...
        if ($regId == 0) {
            foreach ($_POST['selectedRegId'] as $arrId) {
                //del registration
                $query = "DELETE FROM ".DBPREFIX."module_calendar".MODULE_INDEX."_registrations WHERE id='".$arrId."'";
                $objDatabase->Execute($query);

                //del data
                $query = "DELETE FROM ".DBPREFIX."module_calendar".MODULE_INDEX."_form_data WHERE reg_id='".$arrId."'";
                $objDatabase->Execute($query);
            }
        } else {
            //del registration
            $query = "DELETE FROM ".DBPREFIX."module_calendar".MODULE_INDEX."_registrations WHERE id='".$regId."'";
            $objDatabase->Execute($query);

            //del data
            $query = "DELETE FROM ".DBPREFIX."module_calendar".MODULE_INDEX."_form_data WHERE reg_id='".$regId."'";
            $objDatabase->Execute($query);
        }
    }
    ...
    function deleteFormular($noteId)
    {
        ...
        //del formular
        $query = "DELETE FROM ".DBPREFIX."module_calendar".MODULE_INDEX."_form_fields WHERE note_id='".$noteId."'";
        $objDatabase->Execute($query);
    ...
    function _getCsv($id)
    {
        ...
        //note title
        $queryNote         = "SELECT `name` FROM ".DBPREFIX."module_calendar".MODULE_INDEX." WHERE id=".$id."";
            ...
            while (!$objResultReg->EOF) {
                ...
                foreach ($arrFormFields as $arrFieldId => $arrFieldName) {
                    $queryData         = "SELECT `data`
                                       FROM ".DBPREFIX."module_calendar".MODULE_INDEX."_form_data
