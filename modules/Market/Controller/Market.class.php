    ...
    */
    ...
    function entryDetails($id) {
        ...
        if (isset($id) && count($this->entries) != 0) {
            ...
            \Cx\Core\Csrf\Controller\ComponentController::header('Location: ?section=Market');
        }
    }
    ...
    function addEntry()
    {
        ...
        if (!$this->settings['addEntry'] == '1' || (!$this->communityModul && $this->settings['addEntry_only_community'] == '1')) {
            \Cx\Core\Csrf\Controller\ComponentController::header('Location: index.php?section=Market');
            exit;
        }elseif ($this->settings['addEntry_only_community'] == '1') {
            $objFWUser = \FWUser::getFWUserObject();
            if ($objFWUser->objUser->login()) {
                if (!\Permission::checkAccess(99, 'static', true)) {
                    \Cx\Core\Csrf\Controller\ComponentController::header("Location: ".CONTREXX_DIRECTORY_INDEX."?section=Login&cmd=noaccess");
                    exit;
                }
            }else {
                $link = base64_encode(CONTREXX_DIRECTORY_INDEX.'?'.$_SERVER['QUERY_STRING']);
                \Cx\Core\Csrf\Controller\ComponentController::header("Location: ".CONTREXX_DIRECTORY_INDEX."?section=Login&redirect=".$link);
                exit;
            }
        } else {
            $objFWUser = \FWUser::getFWUserObject();
...
//                ($i % 2)? $class = "row2" : $class = "row1";
    ...
    function confirmEntry() {
        ...
        if (isset($_POST['submitEntry']) || isset($_POST['submit'])) {
                ...
                if ($objResult !== false) {
                    ...
                    while(!$objResult->EOF) {
                        ...
                        if ($objResultUpdate !== false) {
                            \Cx\Core\Csrf\Controller\ComponentController::header('Location: ?section=Market&cmd=detail&id='.$objResult->fields['id'].'');
                        }
            ...
            \Cx\Core\Csrf\Controller\ComponentController::header('Location: ?section=Market&cmd=add');
        }
    }
    ...
    function editEntry() {
        ...
        if (!$this->settings['editEntry'] == '1' || (!$this->communityModul && $this->settings['addEntry_only_community'] == '1')) {
            \Cx\Core\Csrf\Controller\ComponentController::header('Location: index.php?section=Market&cmd=detail&id='.$_POST['id']);
            exit;
        }elseif ($this->settings['addEntry_only_community'] == '1') {
            $objFWUser = \FWUser::getFWUserObject();
            if ($objFWUser->objUser->login()) {
...
//                                $i++;
                        ...
                        \Cx\Core\Csrf\Controller\ComponentController::header('Location: index.php?section=Market&cmd=detail&id='.$_GET['id']);
                        exit;
                    }
                }
...
//                        $error = $_CORELANG['TXT_DATABASE_QUERY_ERROR'];
                        \Cx\Core\Csrf\Controller\ComponentController::header('Location: index.php?section=Market&cmd=edit&id='.$_POST['id']);
                        exit;
                    }
                }else{
// TODO: Never used
