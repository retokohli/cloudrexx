...
/**
 * Backend class for the voting system
 * @copyright   CONTREXX CMS - COMVATION AG
 * @author      Contrexx Dev Team <info@comvation.com>
 * @version       1.1
 * @package     contrexx
    ...
    */
    function __construct()
    {
        ...
        $objTemplate->setVariable(array(
            ...
            'CONTENT_NAVIGATION' =>
                "<a href='?cmd=voting'>".$_ARRAYLANG['TXT_VOTING_RESULTS']."</a>
    ...
    function _detail()
    {
        ...
        $objVoting = $objDatabase->SelectLimit('
        ...
        if (!empty($_GET['delete'])) {
            $objMail = $objDatabase->Execute('
                SELECT system_id, voting_id
                FROM `'.DBPREFIX.'voting_rel_email_system`
                WHERE email_id='.intval($_GET['delete']));
            if ($objMail !== false && $objMail->RecordCount() > 0) {
                while (!$objMail->EOF) {
                    $objDatabase->Execute('
                        UPDATE `'.DBPREFIX.'voting_system`
                        SET `votes`=`votes`-1
                        WHERE `id`='.$objMail->fields['system_id']);
                    $objDatabase->Execute('
                        UPDATE `'.DBPREFIX.'voting_results`
                        SET `votes`=`votes`-1
                        WHERE `id`='.$objMail->fields['voting_id'].'
                        AND `voting_system_id`='.$objMail->fields['system_id']);
                    $objMail->MoveNext();
                }
                $objDatabase->Execute('
                    DELETE FROM `'.DBPREFIX.'voting_rel_email_system`
                    WHERE `email_id`='.intval($_GET['delete']));
                $objDatabase->Execute('
                    DELETE FROM `'.DBPREFIX.'voting_email`
                    WHERE `id`='.intval($_GET['delete']));
                $objDatabase->Execute('
                    DELETE FROM `'.DBPREFIX.'voting_additionaldata`
                    WHERE `voting_system_id`='.intval($_GET['delete']));
            }
        }

        if (!empty($_GET['verify'])) {
            $objDatabase->Execute('
                UPDATE `'.DBPREFIX.'voting_email`
                SET `valid`=\'1\'
                WHERE `id`='.intval($_GET['verify']));
        }
        $objCount = $objDatabase->SelectLimit('
            SELECT COUNT(*) AS votecount
            FROM `'.DBPREFIX.'voting_rel_email_system` AS s
            INNER JOIN `'.DBPREFIX.'voting_email` AS e
              ON e.id=s.email_id
            WHERE s.system_id='.$systemId.'
            GROUP BY s.system_id', 1);
        if ($objCount !== false) {
        ...
        $objMails = $objDatabase->SelectLimit('
            SELECT e.id, e.email, e.valid
            FROM `'.DBPREFIX.'voting_rel_email_system` AS s
            INNER JOIN `'.DBPREFIX.'voting_email` AS e
              ON e.id=s.email_id
            WHERE s.system_id='.$systemId.'
            ORDER BY e.email', $_CONFIG['corePagingLimit'], $pos);
        if ($objMails !== false) {
            $row = 1;
            while (!$objMails->EOF) {
                $this->_objTpl->setVariable(array(
     ...
     * @return     iterator
     */
    private function getResultVotes($poll_id)
    {
        global $objDatabase;
     ...
     * @param       int $poll_id
     */
    private function clearAnswers($poll_id)
    {
        global $objDatabase;
     ...
     * @param       array $answer
     */
    private function insertAnswer($poll_id, $answer)
    {
        global $objDatabase;

        $votes = (isset($answer['votes'])) ? intval($answer['votes']) : 0;
        $query = '
            INSERT INTO `'.DBPREFIX.'voting_results` (
                `voting_system_id`,
                `votes`
            ) VALUES (
                '.$poll_id.',
    ...
    private function insertSingleAnswer($id, $langID, $answer)
    {
        ...
        $query = '
            INSERT INTO `'.DBPREFIX.'voting_answer` (
                `resultID`,
                `langID`,
                `answer`
            ) VALUES (
    ...
    private function insertPoll()
    {
        ...
        $method = isset($_POST['votingRestrictionMethod'])
            ...
            : 'cookie';
        $query = '
            INSERT INTO `'.DBPREFIX.'voting_system` (
                `status`,
                `submit_check`,
                `votes`,
                `additional_nickname`,
     ...
     */
    private function insertLang($poll_id, $langID, $title, $question)
    {
        global $objDatabase;
        ...
        $query = '
            INSERT INTO `'.DBPREFIX.'voting_lang` (
                `pollID`,
                `langID`,
                `title`,
                `question`
     ...
     * @param       string $question
     */
    private function updatelang($poll_id, $langID, $title, $question)
    {
        global $objDatabase;
...
// TODO: $poll_id is not defined.  $id maybe?
//        $this->updatePoll($poll_id);
        $this->updatePoll($id);
        $langs = $this->returnLanguages();
        $questions = $_POST['votingquestions'];
        $titles = $_POST['votingnames'];
     ...
     * @param       int $poll_id
     */
    private function updatePoll($poll_id)
    {
        global $objDatabase;
...
//            SET submit_check        = '".$method."',
    ...
    function votingDelete()
    {
        ...
        $query = "
            DELETE FROM `".DBPREFIX."voting_lang`
            WHERE `pollID`=".$id;
        $objDatabase->execute($query);
        $this->clearAnswers($id);
        ...
        $query = "
            DELETE FROM `".DBPREFIX."voting_rel_email_system`
            WHERE system_id=".intval($_GET['votingid']);
        $objDatabase->Execute($query);
        $this->_cleanUpEmails();
        // now delete the actual poll
        $query = "
            DELETE FROM `".DBPREFIX."voting_system`
            WHERE `id`=".$id;
        $objDatabase->execute($query);
...
/*
        ...
        $query = "
            DELETE FROM ".DBPREFIX."voting_results
            WHERE voting_system_id = ".intval($_GET['votingid']);
        $objDatabase->Execute($query);
*/
    }
    ...
    function _cleanUpEmails()
    {
        ...
        $arrEmailIds = array();
        $objEmails = $objDatabase->Execute("
            SELECT e.id
            FROM ".DBPREFIX."voting_email AS e
            INNER JOIN ".DBPREFIX."voting_rel_email_system AS s
            ON s.email_id=e.id");
        if ($objEmails !== false) {
            while (!$objEmails->EOF) {
            ...
            $objDatabase->Execute("
                DELETE FROM ".DBPREFIX."voting_email".
                (count($arrEmailIds) > 0
                    ? " WHERE id!=".implode(' AND id!=', $arrEmailIds)
                    : ''));
        }
    }
...
// TODO: $voltingresults is not defined (and neither is $votingresults...)
//            'VOTING_RESULTS'                        => implode($voltingresults,";"),
        ...
        list($titles, $questions) = $this->getLangValues($poll_id);
     ...
     * @return      array
     */
    private function getLangValues($poll_id)
    {
        global $objDatabase;

        $query = sprintf('
            SELECT `langID`, `title`, `question`
            FROM `%svoting_lang`
            WHERE `pollID`=%s',
            DBPREFIX, $poll_id);
        $res = new DBIterator($objDatabase->execute($query));
     ...
     * @return      array
     */
    private function getAnswers($poll_id)
    {
        global $objDatabase;

        $query = sprintf('
            SELECT `va`.`answer`, `va`.`langID`
            FROM `%svoting_answer` AS `va`
            LEFT JOIN `%svoting_results` AS `vr`
                ON `va`.`resultID`=`vr`.`id`
            WHERE `vr`.`voting_system_id`=%s
            ORDER BY `id`',
            DBPREFIX, DBPREFIX, $poll_id);
        $res = new DBIterator($objDatabase->execute($query));
        $answers = array();
        foreach ($res as $row) {
    ...
    function votingCode()
    {
        ...
        $votingDate=$objResult->fields['datesec'];
        $query = "
            SELECT `vr`.id, `va`.`answer`, `vr`.votes
            FROM `".DBPREFIX."voting_results` AS `vr`
            LEFT JOIN `".DBPREFIX."voting_answer` AS `va`
              ON     `va`.`resultID`=`vr`.`id`
                 AND `va`.`langID`=".BACKEND_LANG_ID."
            WHERE `voting_system_id`=$votingId
            ORDER BY id";
        $objResult = $objDatabase->Execute($query);
        while (!$objResult->EOF) {
