...
class CommonFunctions
{
    ...
    public $_ftpFileWinPCRE = '#^(?:[0-9\-]+)\s+(?:[0-9]{2}:[0-9]{2}(?:AM|PM|))\s+(?:\<DIR\>|[0-9]+|)\s+(.*)$#';
    public $_ftpFileUnixPCRE = '#^(?:[bcdlsp-][rwxtTsS-]{9})\s+(?:[0-9]+)\s+(?:\S+)\s+(?:\S+)\s+(?:[0-9]+)\s+(?:[A-Z][a-z]+\s+[0-9]{1,2}\s+(?:[0-9]{4}|[0-9]{2}:[0-9]{2}|))\s+(.*)$#';
...
//        $errorNr = $db->ErrorNo();
    ...
    */
    ...
    function _isNewestVersion($thisVersion, $newestVersion)
    {
        ...
        for ($nr = 0; $nr < $maxSubVersion; $nr++) {
            ...
            if (!isset($arrNewVersion[$nr])) {
                return true;
            }
            if ($arrNewVersion[$nr] > $arrInstalledVersion[$nr]) {
                return false;
            }
            if ($arrNewVersion[$nr] < $arrInstalledVersion[$nr]) {
                return true;
    ...
    */
    ...
    {
        global $arrDatabaseTables, $_ARRLANG, $sqlDumpFile;
    ...
    {
        global $arrLanguages, $language, $_ARRLANG;
