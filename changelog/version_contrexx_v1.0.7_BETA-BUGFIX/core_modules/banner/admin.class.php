...
class Banner extends bannerLibrary {
    ...
    */    
    function showSettings() {
    	...
    	if (isset($_POST['frmSettings_Active'])) {
    		...
    		$this->statusMessage = $_ARRAYLANG['TXT_BANNER_SETTINGS_SAVED'];
    ...
    */
    function addBanner()
    {
    	...
    	$this->_objTpl->setVariable(array(
    	    ...
    	    'BANNER_ADD_GROUP_MENU'					=> 	$this->getBannerGroupMenu($langId=1, $selectedOption=''),
    ...
    */    
    function insertBanner() {
    	...
    	$strName 	= htmlspecialchars(addslashes($_POST['bannerName']));
    	$intGroupId = intval($_POST['bannerGroupId']);
    	$intStatus	= intval($_POST['bannerStatus']);
    	$strCode	= addslashes($_POST['bannerCode']);
    	...
    	if (!empty($strName)	&&
	    	...
	    	$this->statusMessage = $_ARRAYLANG['TXT_BANNER_INSERT_DONE'];    	    	
    	}
    }
    ...
    */      
    function deleteBanner($intBannerId) {
    	...
    	if ($objResult->RecordCount() == 1) {
    		...
    		$this->statusMessage = $_ARRAYLANG['TXT_BANNER_DELETE_DONE'];
    	}
    ...
    */     
    function editBanner($intBannerId) {
		...
		if ($objResult->RecordCount() > 0) {
			while (!$objResult->EOF) {
	            if (array_key_exists($objResult->fields['catid'],$arrRelationNews)) {
	            	$strSelectedNews .= '<option value="'.$objResult->fields['catid'].'">'.$objResult->fields['name'].' ('.$objResult->fields['catid'].') </option>'."\n";
	            } else {
	            	$strUnselectedNews .= '<option value="'.$objResult->fields['catid'].'">'.$objResult->fields['name'].' ('.$objResult->fields['catid'].') </option>'."\n";
	            }
				$objResult->MoveNext();	
			}
		}           
		...
		if ($objResult->RecordCount() > 0) {
			while (!$objResult->EOF) {
	            if (array_key_exists($objResult->fields['id'],$arrRelationTeaser)) {
	            	$strSelectedTeaser .= '<option value="'.$objResult->fields['id'].'">'.$objResult->fields['name'].' ('.$objResult->fields['id'].') </option>'."\n";
	            } else {
	            	$strUnselectedTeaser .= '<option value="'.$objResult->fields['id'].'">'.$objResult->fields['name'].' ('.$objResult->fields['id'].') </option>'."\n";
	            }
				$objResult->MoveNext();
			}
		}		
    	...
    	$this->_objTpl->setVariable(array(
    		...
    		'BANNER_EDIT_GROUP_MENU'					=>	$this->getBannerGroupMenu($langId=1,$objResult->fields['parent_id']),
    		'BANNER_EDIT_STATUS'						=>	($objResult->fields['status'] == 1) ? 'checked' : '',
    		'BANNER_EDIT_CODE'							=>	$objResult->fields['banner_code'],
    ...
    */       
    function updateBanner() {
    	...
    	if (!empty($strName)	&&
    		...
    		$this->statusMessage = $_ARRAYLANG['TXT_BANNER_UPDATE_DONE'];
    	}
		...
		if ($objResult->RecordCount() > 0) {
			while (!$objResult->EOF) {
				...
				$this->_objTpl->setVariable(array(
		            ...
		            'BANNER_LIVE'				=> $objResult->fields['banner_code'],
		            'BANNER_CODE'				=> htmlspecialchars($objResult->fields['banner_code']),
    ...
    */     
    function emptyGroup($intGroupId) {
    	...
    	$this->statusMessage = $_ARRAYLANG['TXT_BANNER_EMPTY_GROUP_DONE'];
    }
    ...
    */    
    function updateGroup() {
    	...
    	$strName	= htmlspecialchars(addslashes($_POST['groupName']));
    	$strDesc	= htmlspecialchars(addslashes($_POST['groupDesc']));
    	...
    	$this->statusMessage = $_ARRAYLANG['TXT_BANNER_GROUP_UPDATE'];	
    }
