...
*
* @copyright     ASTALAVISTA SECURE CMS - 2004 Astalavista Group GmbH 
...
class calendarManager extends calendarLibrary
{
    ...
    function getCalendarPage()
    {
		...
		$objTemplate->setVariable(array(
			'CONTENT_STATUS_MESSAGE'	=> ltrim($this->statusMessage),
...
//		$objDatabase->debug = true;
		...
		if (isset($_POST['search'])) {
			$keyword = htmlentities(addslashes($_POST['inputKeyword']));
		...
		if ($objDatabase->Affected_Rows() > 0) {
			while(!$objResult->EOF) {					
				...
				$this->_objTpl->setVariable(array(
					...
					'CALENDAR_EVENT_TITLE'		=> htmlentities($objResult->fields['name']),
	...
	function delNote($id)
	{
		...
		$this->statusMessage = $_ARRAYLANG['TXT_CALENDAR_STAT_DEL']."<br />";
	}
...
//			'TXT_CALENDAR_PRIORITY'	     => $_ARRAYLANG['TXT_CALENDAR_PRIORITY'],
	...
	function writeEditedNote()
	{
		...
		if ($objDatabase->Affected_Rows() > 0) {
			...
			$this->statusMessage = $_ARRAYLANG['TXT_CALENDAR_STAT_EDITED']."<br />";
		} else {
			$this->statusMessage = $_ARRAYLANG['TXT_CALENDAR_STAT_ERROR_EXISTING']."<br />";
		}
	}
	...
	function showCategories()
	{
	    ...
	    if (isset($_GET['new']) and $_GET['new'] == 1){
	    	if ($_POST['name'] != '' and $_POST['status'] != '' and $_POST['lang']){
				...
				if ($objDatabase->Affected_Rows() == 0){
		    		...
		    		$objDatabase->Execute($query);
		    		$this->statusMessage = $_ARRAYLANG['TXT_CALENDAR_MESSAGE_NEW_CATRGORY'];
				}else{
					$this->statusMessage = $_ARRAYLANG['TXT_CALENDAR_MESSAGE_ERROR_EXISTING'];
				}
	    	}else{
	    		$this->statusMessage = $_ARRAYLANG['TXT_CALENDAR_MESSAGE_ERROR_FILL_IN_ALL'];
	    	}
	    }
	    ...
	    if (isset($_GET['chg']) and $_GET['chg'] == 1 and isset($_POST['selected']) and is_array($_POST['selected'])) {
	    	...
	    	if ($_POST['form_delete'] != ''){
				...
				if ($x > 0){
					$this->statusMessage = $_ARRAYLANG['TXT_CALENDAR_CAT_RECORDS'];
				} else {
					$this->statusMessage = $_ARRAYLANG['TXT_CALENDAR_CAT_DELETE'];
				}
			...
			if ($_POST['form_activate'] != '' or $_POST['form_deactivate'] != ''){
				...
				$_SESSION['statusMessage'] = $_ARRAYLANG['TXT_CALENDAR_CAT_CHANGE_STATUS'];
		...
		if (isset($_GET['chg']) and $_GET['chg'] == 1 and $_POST['form_sort'] == 1){
			...
			$this->statusMessage = $_ARRAYLANG['TXT_CALENDAR_SORTING_COMPLETE'];
		}
		...
		if (isset($_GET['set']) and $_GET['set'] == 1) {
			if ($_POST['id'] != '' and $_POST['name'] != '' and $_POST['status'] != '' and $_POST['lang']) {				
				...
				if ($objDatabase->affected_rows() == 0){			
					...
					$this->statusMessage = $_ARRAYLANG['TXT_CALENDAR_MESSAGE_SUCCESSFULL_EDIT_CAT'];
				} else {
					$this->statusMessage = $_ARRAYLANG['TXT_CALENDAR_MESSAGE_ERROR_EXISTING'];
				}
			} else {
...
//		
	...
	function setStdCat()
	{
		...
		$this->statusMessage = $_ARRAYLANG['TXT_CALENDAR_STYLE_MODIFIED']."<br />";
	}
