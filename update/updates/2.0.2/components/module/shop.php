...
function _shopUpdate()
{
    ...
    if (!isset($arrIndexes['group_id']['columns'])) {
        $query = "
            ALTER TABLE ".DBPREFIX."module_shop_products
            ADD INDEX (group_id)
        ";
    ...
    if (!isset($arrIndexes['article_id']['columns'])) {
        $query = "
            ALTER TABLE ".DBPREFIX."module_shop_products
    ...
    if (!isset($arrIndexes['keywords']['columns'])) {
        $query = "
            ALTER TABLE ".DBPREFIX."module_shop_products
        ...
        if ($objResult->RecordCount() == 0) {
            $query = "
                INSERT INTO `".DBPREFIX."module_shop_mail` (
                    ...
                    '4', 'Bestellungsbestaetigung mit Zugangsdaten', '1'
                );
            ";
            $objResult = $objDatabase->Execute($query);
            if ($objResult) {
    ...
    if ($objResult) {
        if ($objResult->RecordCount() == 0) {
            $query = "SELECT `value` FROM `".DBPREFIX."module_shop_config` WHERE `name` = 'email'";
            ...
            if ($objResult) {
                $query = "
                    INSERT INTO `".DBPREFIX."module_shop_mail_content` (
                        ...
                        '4', '1', '".addslashes($objResult->fields['value'])."', 'Contrexx Demo Online Shop',
