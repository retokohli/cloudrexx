...
function _utf8Update()
{
    ...
    try {
        ...
        if (!preg_match('/COLLATE=([a-z_0-9]*)/', $createStatement, $matches)) {
            setUpdateMsg('Die Kollation der Datenbank konnte nicht ausfindig gemacht werden.');
            return false;
        }
        ...
        if (in_array($table, array_keys($arrInstalledTables))) {
            if ($arrInstalledTables[$table] == $_SESSION['contrexx_update']['update']['core']['utf8_collation']) {
                ...
                if (!in_array($table.'_new', $arrInstalledTables)) {
                    ...
                    $query = $objTableStructure->fields['Create Table']." DEFAULT CHARSET=utf8 COLLATE=".$objUpdate->addslashes($_SESSION['contrexx_update']['update']['core']['utf8_collation']).";\n";
                    if ($objDatabase->Execute($query) === false) {
                            return _databaseError($query, $objDatabase->ErrorMsg());
                    }
                }
    ...
    // either TRUE or 'charset_changed' in case the charset/collation has been changed
    return $result;
}
